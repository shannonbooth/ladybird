...
void ConnectionFromClient::host_navigable(Compositing::PageId page_id, Web::HTML::CrossProcessId navigable_id, Web::HTML::SessionHistoryEntryDescriptor current_history_entry)
{
    if (auto page = this->page(page_id); page.has_value()) {
        page->page().host_navigable(navigable_id, current_history_entry);
        page->page().client().request_frame();
    }
}
