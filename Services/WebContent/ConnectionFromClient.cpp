...
void ConnectionFromClient::debug_request(u64 page_id, ByteString request, ByteString argument)
{
    ...
    if (request == "dump-display-list") {
        if (auto* doc = page->page().local_root_navigable()->active_document().ptr()) {
    ...
    if (request == "dump-local-storage") {
        if (auto* document = page->page().local_root_navigable()->active_document().ptr()) {
...
void ConnectionFromClient::inspect_storage(u64 page_id, Web::StorageAPI::StorageEndpointType storage_endpoint, u64 request_id)
{
    ...
    auto* document = page->page().local_root_navigable()->active_document().ptr();
...
void ConnectionFromClient::remove_dom_node(u64 page_id, Web::UniqueNodeID node_id)
{
    ...
    auto* active_document = page->page().local_root_navigable()->active_document().ptr();
    if (!active_document) {
        async_did_finish_editing_dom_node(page_id, {});
        return;
    }
...
{
    auto* document = page.local_root_navigable()->active_document().ptr();
    if (!document) {
