...
void PageClient::page_did_update_cookie(HTTP::Cookie::Cookie const& cookie)
{
    ...
    if (auto* document = page().local_root_navigable()->active_document().ptr())
...
{
    auto* active_document = page().local_root_navigable()->active_document().ptr();
...
Vector<Web::CSS::StyleSheetIdentifier> PageClient::list_style_sheets() const
{
    ...
    auto const* document = page().local_root_navigable()->active_document().ptr();
    if (document) {
