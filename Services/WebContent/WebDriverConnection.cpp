...
Messages::WebDriverClient::NavigateToResponse WebDriverConnection::navigate_to(JsonValue payload)
{
    ...
    auto will_replace_web_content_process = current_top_level_browsing_context()->page().client().decide_navigation_process(current_url, url.value()) == Web::NavigationProcessDecision::Remote;
...
Messages::WebDriverClient::LoadUrlFromUiResponse WebDriverConnection::load_url_from_ui(JsonValue payload)
{
    ...
    auto const& current_url = current_top_level_browsing_context()->active_document()->url();
    auto will_replace_web_content_process = current_top_level_browsing_context()->page().client().decide_navigation_process(current_url, *url) == Web::NavigationProcessDecision::Remote;
