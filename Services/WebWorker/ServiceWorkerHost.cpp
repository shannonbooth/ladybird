...
ServiceWorkerHost::ServiceWorkerHost(URL::URL url)
    : m_url(move(url))
{
}
...
{
    // FIXME: This should be serialized over IPC.
    Web::ServiceWorker::SerializedServiceWorkerRecord service_worker;
    ...
    auto settings_object = Web::ServiceWorker::ServiceWorkerEnvironmentSettingsObject::setup(page, *worker_global_scope, move(realm_execution_context), unsafe_worker_creation_time);
    ...
    settings_object->creation_url = service_worker.script_url;
    ...
    worker_global_scope->set_url(service_worker.script_url);
