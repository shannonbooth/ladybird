...
void ConnectionFromClient::adopt_request(int source_client_id, u64 source_request_id, u64 target_request_id, bool preserve_transfer_lease)
{
    ...
    RefPtr<ConnectionFromClient> source_connection;
    auto current_request_id = source_request_id;
    if (transfer_lease.has_value()) {
        source_connection = transfer_lease->owner;
        current_request_id = transfer_lease->request_id;
    } else if (auto connection = m_connections.get(source_client_id); connection.has_value()) {
    ...
    if (preserve_transfer_lease) {
        if (transfer_lease.has_value()) {
            ...
            m_request_transfer_leases.set(lease_key, RequestTransferLease { *this, target_request_id });
        }
    } else if (transfer_lease.has_value()) {
...
{
    auto lease_key = Requests::RequestTransferLeaseKey { source_client_id, source_request_id };
    auto lease = m_request_transfer_leases.take(lease_key);
    if (!lease.has_value())
        return;
