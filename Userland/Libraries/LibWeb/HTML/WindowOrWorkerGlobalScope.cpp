...
// https://whatpr.org/html/9893/b8ea975...df5706b/timers-and-user-prompts.html#timer-initialisation-steps
...
i32 WindowOrWorkerGlobalScopeMixin::run_timer_initialization_steps(TimerHandler handler, i32 timeout, JS::MarkedVector<JS::Value> arguments, Repeat repeat, Optional<i32> previous_id)
{
    ...
    auto task = JS::create_heap_function(vm.heap(), Function<void()>([this, &realm, handler = move(handler), timeout, arguments = move(arguments), repeat, id, initiating_script]() {
        ...
        handler.visit(
            ...
            [&](String const& source) {
                ...
                auto script = ClassicScript::create(basename, source, realm, move(base_url));
