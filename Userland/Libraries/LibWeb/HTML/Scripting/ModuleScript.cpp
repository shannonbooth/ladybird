...
// https://whatpr.org/html/9893/b8ea975...df5706b/webappapis.html#creating-a-javascript-module-script
WebIDL::ExceptionOr<JS::GCPtr<JavaScriptModuleScript>> JavaScriptModuleScript::create(ByteString const& filename, StringView source, JS::Realm& realm, URL::URL base_url)
{
    ...
    for (auto const& requested : result.value()->requested_modules()) {
        ...
        // 5. If the result of running the module type allowed steps given moduleMapRealm and settings is false, then:
        if (!module_type_allowed(module_type, realm)) {
...
// https://whatpr.org/html/9893/b8ea975...df5706b/webappapis.html#run-a-module-script
