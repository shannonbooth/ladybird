...
WebIDL::ExceptionOr<JS::NonnullGCPtr<PendingResponse>> nonstandard_resource_loader_file_or_http_network_fetch(JS::Realm& realm, Infrastructure::FetchParams const& fetch_params, IncludeCredentials include_credentials, IsNewConnectionFetch is_new_connection_fetch)
{
    ...
    if (request->buffer_policy() == Infrastructure::Request::BufferPolicy::DoNotBufferResponse) {
        ...
        auto on_headers_received = JS::create_heap_function(realm.heap(), [&vm, request, pending_response, stream](HTTP::HeaderMap const& response_headers, Optional<u32> status_code) {
        ...
        auto on_data_received = JS::create_heap_function(realm.heap(), [fetched_data_receiver](ReadonlyBytes bytes) {
        ...
        auto on_complete = JS::create_heap_function(realm.heap(), [&vm, &realm, pending_response, stream](bool success, Optional<StringView> error_message) {
            HTML::TemporaryExecutionContext execution_context { Bindings::host_defined_environment_settings_object(realm), HTML::TemporaryExecutionContext::CallbacksEnabled::Yes };
        ...
        auto on_load_success = JS::create_heap_function(realm.heap(), [&realm, &vm, request, pending_response](ReadonlyBytes data, HTTP::HeaderMap const& response_headers, Optional<u32> status_code) {
            (void)request;
        ...
        auto on_load_error = JS::create_heap_function(realm.heap(), [&realm, &vm, request, pending_response](ByteString const& error, Optional<u32> status_code, ReadonlyBytes data, HTTP::HeaderMap const& response_headers) {
            (void)request;
