...
static String generate_a_segment_wildcard_regexp(PatternParser::Options const& options)
{
    // 1. Let result be "[^".
    ...
    Token const* regexp_or_wildcard_token, String const& suffix, Token const* modifier_token)
{
    ...
    if (is_a_duplicate_name(name))
        return ErrorInfo { "Duplicate name provided in URL pattern"_string }; // FIXME: Improve error
