...
use crate::ffi::{RustFfiUrl, RustUrlByteSlice};
use crate::pattern::{
    ComponentResult, GroupMatch, IgnoreCase, Init, Input, MatchInput, Pattern, PatternErrorOr,
...
fn optional_string_from_raw(ptr: *const u8, len: usize) -> Option<String> {
    ...
    Some(decode_utf8(RustUrlByteSlice {
        data: ptr,
        length: len,
    }))
}
...
pub unsafe extern "C" fn rust_url_pattern_create_from_string(
    ...
    abort_on_panic(|| {
        ...
        match Pattern::create(
            &Input::String(input_str.to_string()),
            &base_url,
            ignore_case,
        ) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_url_pattern_has_regexp_groups(
...
pub unsafe extern "C" fn rust_url_pattern_exec_string(
    ...
    abort_on_panic(|| {
        ...
        exec_match_result(
            pattern
                .0
                .r#match(&MatchInput::String(input_str.to_string()), &base_url),
            error_out,
            error_len_out,
            ctx,
            on_complete,
...
/// `pattern` and `input` must be valid pointers.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_url_pattern_test_url(
    pattern: *const RustUrlPattern,
    input: *const RustFfiUrl,
) -> bool {
    abort_on_panic(|| {
        if pattern.is_null() || input.is_null() {
            return false;
        ...
        matches!(
            pattern.0.r#match(&MatchInput::Url(input), &None),
            Ok(Some(_))
        )
    })
}
