...
use crate::pattern::tokenizer::{Policy as TokenizerPolicy, Type as TokenType};
use crate::pattern::{
    Component, Init, Options, PatternErrorOr, Token, Tokenizer, canonicalize_a_protocol,
...
impl ConstructorStringParser {
    ...
    pub(crate) fn state_to_string(&self) -> &'static str {
        match self.state {
    ...
    pub fn parse(input: &str) -> PatternErrorOr<Init> {
        ...
        while (parser.token_index as usize) < parser.token_list.len() {
            ...
            if parser.token_list[parser.token_index as usize].type_ == TokenType::End {
    ...
    pub(crate) fn compute_protocol_matches_a_special_scheme_flag(&mut self) -> PatternErrorOr<()> {
        ...
        let protocol_matches_special_scheme =
            protocol_string_matches_a_special_scheme(&protocol_string)?;
        ...
        if protocol_matches_special_scheme {
    ...
    pub(crate) fn result_for_active_state(&self) -> Option<&String> {
        match self.state {
            State::Protocol => self.result.protocol.as_ref(),
            State::Username => self.result.username.as_ref(),
            State::Password => self.result.password.as_ref(),
            State::Hostname => self.result.hostname.as_ref(),
            State::Port => self.result.port.as_ref(),
            State::Pathname => self.result.pathname.as_ref(),
            State::Search => self.result.search.as_ref(),
            State::Hash => self.result.hash.as_ref(),
    ...
    pub(crate) fn is_a_group_open(&self) -> bool {
        ...
        if self.token_list[self.token_index as usize].type_ == TokenType::Open {
            return true;
        }
    ...
    pub(crate) fn is_a_group_close(&self) -> bool {
        ...
        if self.token_list[self.token_index as usize].type_ == TokenType::Close {
            return true;
        }
...
fn protocol_string_matches_a_special_scheme(protocol_string: &str) -> PatternErrorOr<bool> {
    ...
    Ok(protocol_component_matches_a_special_scheme(
        &protocol_component,
    ))
}
