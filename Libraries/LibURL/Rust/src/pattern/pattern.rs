...
use crate::pattern::{
    Component, ConstructorStringParser, Init, PatternErrorOr, PatternProcessType,
    ...
    canonicalize_an_ipv6_hostname, canonicalize_an_opaque_pathname, process_a_url_pattern_init,
    protocol_component_matches_a_special_scheme,
};
use crate::url::{
    BasicParseOptions, ExcludeFragment, Url, basic_parse, default_port_for_scheme,
    is_special_scheme,
};
...
impl Pattern {
    ...
    pub fn create(
        ...
        // 3. Otherwise:
            ...
            let Input::Init(input_init) = input else {
                unreachable!()
            };
        ...
        if is_special_scheme(processed_init.protocol.as_ref().unwrap().as_bytes())
            ...
            && Some(maybe_port)
                == default_port_for_scheme(processed_init.protocol.as_ref().unwrap())
        {
    ...
    pub fn r#match(
        &self,
        input: &MatchInput,
        base_url_string: &Option<String>,
    ) -> PatternErrorOr<Option<Result>> {
        ...
        result.port = self
            .port_component
            .create_match_result(&port, &port_exec_result);
        ...
        result.search = self
            .search_component
            .create_match_result(&search, &search_exec_result);
        ...
        result.hash = self
            .hash_component
            .create_match_result(&hash, &hash_exec_result);
