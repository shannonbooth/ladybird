...
use libunicode_rust::{
    code_point_has_identifier_continue_property, code_point_has_identifier_start_property,
};

use crate::pattern::{ErrorInfo, PatternErrorOr};
...
impl Token {
    pub fn type_to_string(type_: Type) -> &'static str {
        match type_ {
            Type::Open => "Open",
            Type::Close => "Close",
            Type::Regexp => "Regexp",
...
impl fmt::Display for Token {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "{}, index: {}, value: '{}'",
            Self::type_to_string(self.type_),
            self.index,
            self.value
        )
    }
...
impl Tokenizer {
        ...
        while tokenizer.index < tokenizer.input_code_points.len() {
            ...
            if tokenizer.code_point == '\\' as u32 {
                ...
                if tokenizer.index == tokenizer.input_code_points.len() - 1 {
                    ...
                    tokenizer.process_a_tokenizing_error(
                        tokenizer.next_index as u32,
                        tokenizer.index as u32,
                    )?;
            ...
            if tokenizer.code_point == ':' as u32 {
                ...
                while name_position < tokenizer.input_code_points.len() {
                    ...
                    let valid_code_point =
                        Self::is_a_valid_name_code_point(tokenizer.code_point, first_code_point);
                ...
                if name_position <= name_start {
                    ...
                    tokenizer
                        .process_a_tokenizing_error(name_start as u32, tokenizer.index as u32)?;
            ...
            if tokenizer.code_point == '(' as u32 {
                ...
                if depth != 0 {
                    // 1. Run process a tokenizing error given tokenizer, regexp start, and tokenizer’s index.
                    tokenizer
                        .process_a_tokenizing_error(regexp_start as u32, tokenizer.index as u32)?;
    ...
    pub(crate) fn add_a_token_with_default_length(
        &mut self,
        type_: Type,
        next_position: u32,
        value_position: u32,
        ...
        self.add_a_token(type_, next_position, value_position, computed_length);
    }

    // https://urlpattern.spec.whatwg.org/#add-a-token-with-default-position-and-length
    pub(crate) fn add_a_token_with_default_position_and_length(&mut self, type_: Type) {
        // 1. Run add a token with default length given tokenizer, type, tokenizer’s next index, and tokenizer’s index.
        self.add_a_token_with_default_length(type_, self.next_index as u32, self.index as u32);
    }

    // https://urlpattern.spec.whatwg.org/#process-a-tokenizing-error
    pub(crate) fn process_a_tokenizing_error(
        &mut self,
        next_position: u32,
        value_position: u32,
    ) -> PatternErrorOr<()> {
