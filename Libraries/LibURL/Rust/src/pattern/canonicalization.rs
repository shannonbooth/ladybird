...
use crate::url::{BasicParseOptions, State, Url, basic_parse};
...
pub fn canonicalize_a_protocol(value: &str) -> PatternErrorOr<String> {
    ...
    let parse_result = basic_parse(
        &format!("{value}://dummy.invalid"),
        BasicParseOptions::new(),
    );
...
pub fn canonicalize_an_ipv6_hostname(value: &str) -> PatternErrorOr<String> {
    ...
    for code_point in value.chars() {
        ...
        if !code_point.is_ascii_hexdigit()
            && code_point != '['
            && code_point != ']'
            && code_point != ':'
        {
            return Err(ErrorInfo::new(
                "Failed to canonicalize IPv6 hostname string",
            ));
        }
...
pub fn canonicalize_a_port(
    port_value: &str,
    protocol_value: &Option<String>,
...
pub fn canonicalize_a_pathname(value: &str) -> String {
    ...
    let _ = basic_parse(
        ...
        BasicParseOptions::new()
            .url(&mut dummy_url)
            .state_override(State::PathStart),
    );
...
pub fn canonicalize_an_opaque_pathname(value: &str) -> PatternErrorOr<String> {
    ...
    let parse_result = basic_parse(
        ...
        BasicParseOptions::new()
            .url(&mut dummy_url)
            .state_override(State::OpaquePath),
    );

    // 5. If parseResult is failure, then throw a TypeError.
    if parse_result.is_none() {
        return Err(ErrorInfo::new(
            "Failed to canonicalize opaque pathname string",
        ));
    }
