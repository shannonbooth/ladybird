...
use crate::pattern::part::{Modifier as PartModifier, Type as PartType};
use crate::pattern::tokenizer::{Policy as TokenizerPolicy, Type as TokenType};
use crate::pattern::{
    ErrorInfo, Options, Part, PatternErrorOr, Token, Tokenizer, full_wildcard_regexp_value,
...
impl PatternParser {
    ...
    pub(crate) fn add_a_part(
        ...
        let mut regexp_value =
            ...
            if let Some(regexp_or_wildcard_token) = regexp_or_wildcard_token.as_ref() {
                ...
                if regexp_or_wildcard_token.type_ == TokenType::Asterisk {
        ...
        if regexp_value == self.segment_wildcard_regexp {
            ...
            type_ = PartType::SegmentWildcard;
    ...
    pub(crate) fn try_to_consume_a_regexp_or_wildcard_token(
        &mut self,
        name_token: Option<Token>,
    ) -> Option<Token> {
    ...
    pub(crate) fn try_to_consume_a_token(&mut self, type_: TokenType) -> Option<Token> {
    ...
    pub fn parse(
        ...
        while parser.index < parser.token_list.len() {
            ...
            let mut regexp_or_wildcard_token =
                parser.try_to_consume_a_regexp_or_wildcard_token(name_token.clone());
            ...
            if name_token.is_some() || regexp_or_wildcard_token.is_some() {
                ...
                if !prefix.is_empty()
                    && (options.prefix_code_point.is_none()
                        || prefix != options.prefix_code_point.unwrap().to_string())
                {
                    // 1. Append prefix to the end of parser’s pending fixed value.
