...
use crate::pattern::{Options, Part};
...
pub fn generate_a_pattern_string(part_list: &[Part], options: &Options) -> String {
    ...
    for index in 0..part_list.len() {
        ...
        if part.type_ == PartType::FixedText {
        ...
        if !needs_grouping
            && custom_name
            && part.type_ == PartType::SegmentWildcard
            ...
            && next_part.suffix.is_empty()
        {
            ...
            if next_part.type_ == PartType::FixedText {
                ...
                needs_grouping = next_part.value.chars().next().is_some_and(|code_point| {
                    Tokenizer::is_a_valid_name_code_point(code_point as u32, false)
                });
            }
            // 2. Otherwise:
            else {
        ...
        if !needs_grouping
            ...
            && previous_part.unwrap().type_ == PartType::FixedText
            && ((previous_part.unwrap().value.is_empty() && options.prefix_code_point.is_none())
                || (options.prefix_code_point.is_some()
                    && previous_part.unwrap().value
                        == options.prefix_code_point.unwrap().to_string()))
        {
            needs_grouping = true;
        }
        ...
        else if part.type_ == PartType::SegmentWildcard && !custom_name {
            // 1. Append "(" to the end of result.
            result.push('(');
        ...
        else if part.type_ == PartType::FullWildcard {
        ...
        //     then append U+005C (\) to the end of result.
        if part.type_ == PartType::SegmentWildcard
            && custom_name
            && !part.suffix.is_empty()
            && part.suffix.chars().next().is_some_and(|code_point| {
                Tokenizer::is_a_valid_name_code_point(code_point as u32, false)
            })
        {
            result.push('\\');
