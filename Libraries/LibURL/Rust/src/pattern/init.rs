...
use crate::pattern::{
    ErrorInfo, PatternErrorOr, canonicalize_a_hash, canonicalize_a_hostname,
    canonicalize_a_password, canonicalize_a_pathname, canonicalize_a_port, canonicalize_a_protocol,
    canonicalize_a_search, canonicalize_a_username, canonicalize_an_opaque_pathname,
    escape_a_pattern_string,
};
use crate::url::{BasicParseOptions, basic_parse, is_special_scheme};
...
fn process_a_base_url_string(input: &str, type_: PatternProcessType) -> String {
    // 1. Assert: input is not null.
    // 2. If type is not "pattern" return input.
    if type_ != PatternProcessType::Pattern {
...
fn is_an_absolute_pathname(input: &str, type_: PatternProcessType) -> bool {
    ...
    if type_ == PatternProcessType::Url {
        return false;
    }
...
fn process_protocol_for_init(value: &str, type_: PatternProcessType) -> PatternErrorOr<String> {
...
fn process_username_for_init(value: &str, type_: PatternProcessType) -> String {
...
fn process_password_for_init(value: &str, type_: PatternProcessType) -> String {
    // 1. If type is "pattern" then return value.
    if type_ == PatternProcessType::Pattern {
        return value.to_string();
    }
...
fn process_hostname_for_init(value: &str, type_: PatternProcessType) -> PatternErrorOr<String> {
    // 1. If type is "pattern" then return value.
    if type_ == PatternProcessType::Pattern {
...
fn process_search_for_init(value: &str, type_: PatternProcessType) -> String {
...
fn process_hash_for_init(value: &str, type_: PatternProcessType) -> String {
...
pub fn process_a_url_pattern_init(
    ...
    if let Some(init_base_url) = &init.base_url {
        ...
        if init.protocol.is_none() {
            result.protocol = Some(process_a_base_url_string(&base_url_ref.scheme, type_));
        }
        ...
        {
            result.username = Some(process_a_base_url_string(&base_url_ref.username, type_));
        }
        ...
        {
            result.password = Some(process_a_base_url_string(&base_url_ref.password, type_));
        }
        ...
        if init.protocol.is_none() && init.hostname.is_none() {
            ...
            result.hostname = Some(process_a_base_url_string(&base_host, type_));
        }
        ...
        {
            result.pathname = Some(process_a_base_url_string(
                &base_url_ref.serialize_path(),
                type_,
            ));
        }
            ...
            && init.search.is_none()
        {
            ...
            result.search = Some(process_a_base_url_string(
                base_query.as_deref().unwrap_or(""),
                type_,
            ));
        }
    ...
    if let Some(protocol) = &init.protocol {
        result.protocol = Some(process_protocol_for_init(protocol, type_)?);
    }
    ...
    if let Some(username) = &init.username {
        result.username = Some(process_username_for_init(username, type_));
    }
    ...
    if let Some(password) = &init.password {
        result.password = Some(process_password_for_init(password, type_));
    }
    ...
    if let Some(hostname) = &init.hostname {
        result.hostname = Some(process_hostname_for_init(hostname, type_)?);
    }
    ...
    if let Some(port) = &init.port {
        result.port = Some(process_port_for_init(port, &result_protocol_string, type_)?);
    }
    ...
    if let Some(init_pathname) = &init.pathname {
        ...
        if let Some(base_url) = base_url.as_ref()
            ...
            && !is_an_absolute_pathname(result.pathname.as_deref().unwrap(), type_)
        {
            ...
            let base_url_path = process_a_base_url_string(&base_url.serialize_path(), type_);
    ...
    if let Some(search) = &init.search {
        result.search = Some(process_search_for_init(search, type_));
    }
    ...
    if let Some(hash) = &init.hash {
        result.hash = Some(process_hash_for_init(hash, type_));
    }
