...
use libunicode_rust::{ToAsciiOptions, idna_to_ascii};

use super::parser::report_validation_error;
use super::percent_encoding::{PercentEncodeSet, percent_decode, percent_encode};
...
fn parse_opaque_host(input: &str) -> Option<Host> {
    ...
    Some(Host::Opaque(percent_encode(
        input,
...
fn ends_in_a_number_checker(input: &str) -> bool {
    ...
    last.len() >= 2
        ...
        && last[2..]
            .chars()
            .all(|code_point| code_point.is_ascii_hexdigit())
}
