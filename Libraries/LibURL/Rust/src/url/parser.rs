...
use super::percent_encoding::{
    PercentEncodeSet, append_percent_encoded_if_necessary, output_encoding, percent_encode,
    percent_encode_after_encoding,
};
use super::{BasicParseOptions, Host, State, Url, default_port_for_scheme, is_special_scheme};
...
fn contains_ascii_tab_or_newline(input: &str) -> bool {
    input
        .chars()
        .any(|code_point| matches!(code_point, '\t' | '\n' | '\r'))
}
...
pub(super) fn url_includes_credentials(_url: &Url) -> bool {
    // A URL includes credentials if its username or password is not the empty string.
    !_url.username.is_empty() || !_url.password.is_empty()
}
...
pub(super) fn cloned_shortened_path(base_url: &Url) -> Vec<String> {
    if base_url.scheme == "file"
        && base_url.path.len() == 1
        && is_normalized_windows_drive_letter(&base_url.path[0])
    {
        ...
        match state {
            ...
            State::SchemeStart => {
                ...
                if let Some(code_point) =
                    code_point.filter(|code_point| code_point.is_ascii_alphabetic())
                {
            ...
            State::Scheme => {
                ...
                if let Some(code_point) = code_point.filter(|code_point| {
                ...
                // 2. Otherwise, if c is U+003A (:), then:
                    ...
                    if options.state_override.is_some() {
                        ...
                        if url.scheme == "file"
                            && url.host.as_ref().is_some_and(Host::is_empty_host)
                        {
                            return true;
                        }
                    }
                    ...
                    // 6. Otherwise, if url is special, base is non-null, and base’s scheme is url’s scheme:
                        ...
                        && options
                            .base_url
                            .is_some_and(|base_url| base_url.scheme == url.scheme)
                    {
            ...
            State::Relative => {
                ...
                let base_url = options
                    .base_url
                    .expect("relative state requires a base URL");
            ...
            State::RelativeSlash => {
                ...
                let base_url = options
                    .base_url
                    .expect("relative slash state requires a base URL");
            ...
            State::Authority => {
                ...
                let authority_end = remaining.char_indices().find_map(|(index, byte)| {
                    (matches!(byte, '@' | '/' | '?' | '#') || (url.is_special() && byte == '\\'))
                        .then_some(index)
                });
                ...
                if delimiter_code_point == Some('@') {
                    ...
                    if password_token_seen {
                        let encoded_authority =
                            percent_encode(authority, PercentEncodeSet::Userinfo, false);
                        password_builder.push_str(&encoded_authority);
                    } else if let Some(password_end) = authority.find(':') {
                        password_token_seen = true;

                        let encoded_username = percent_encode(
                            &authority[..password_end],
                            PercentEncodeSet::Userinfo,
                            false,
                        );
                        let encoded_password = percent_encode(
                            &authority[password_end + 1..],
                            PercentEncodeSet::Userinfo,
                            false,
                        );
            ...
            State::File => {
                ...
                // 4. Otherwise, if base is non-null and base’s scheme is "file":
                    ...
                    .base_url
                    .is_some_and(|base_url| base_url.scheme == "file")
                {
            ...
            State::Query => {
                ...
                if encoding != "utf-8"
                    && (!url.is_special() || url.scheme == "ws" || url.scheme == "wss")
                {
                ...
                // then:
                if (options.state_override.is_none() && code_point == Some('#'))
                    || code_point.is_none()
                {
                    ...
                    let query = percent_encode_after_encoding(
                        &encoding,
                        &buffer,
            ...
            State::Fragment => {
                ...
                if let Some(byte) = code_point {
                    ...
                    let fragment = percent_encode_after_encoding(
                        "utf-8",
                        &fragment_input,
