...
ErrorOr<SiteIsolationManager::DocumentHost> SiteIsolationManager::obtain_child_document_host(CanonicalNavigable& navigable, CanonicalSimilarOriginWindowAgent& agent)
{
    ...
    if (host) {
        ...
        host->async_create_embedded_page(page_id, navigable.remote_ancestor_descriptors(), navigable.id(),
        ...
        auto process = TRY(Application::the().launch_child_frame_web_content_process(navigable.reporting_client().is_private(), navigable.remote_ancestor_descriptors(), navigable.id(), current_entry->document_state.id));
