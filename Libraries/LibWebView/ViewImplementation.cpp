...
void ViewImplementation::replace_web_content_process_for_top_level_traversal()
{
    // NB: WebDriver's connection follows the WebContent process. Without this handshake, the old
    //     process's death reads as the window closing and ends the WebDriver session before the
    //     replacement process can attach. Navigation-initiated swaps perform the same handshake in
    //     PageClient::request_new_process_for_navigation, for which this notification is a no-op.
    if (m_client_state.client)
        client().notify_webdriver_window_replacement_started(page_id());
...
void ViewImplementation::reload()
{
    if (m_is_showing_crash_page) {
        ...
        m_should_suppress_history_for_current_load = false;
        m_should_suppress_history_for_next_load = false;
        // NB: The crash page is a live document that is not part of the session history, so the current entry is
        //     recovered the same way a crash is: a replacement process is seeded and traverses to the current step.
        replace_web_content_process_for_top_level_traversal();
        auto crash_recovery = m_top_level_traversable.prepare_to_reload_from_crash_page();
        m_webdriver_pending_navigation_url = m_url;
        m_webdriver_pending_navigation_completes_with_session_history_update = !crash_recovery.should_restore_current_entry_from_ui_process;
        seed_web_content_session_history_from_ui_process();
        if (crash_recovery.should_restore_current_entry_from_ui_process)
    ...
    Function<void(HistoryTraversalOutcome)> on_cancelation_check_complete)
{
    ...
    case HistoryTraversalAction::CheckForCancelation:
        ...
        return decision.outcome;
    case HistoryTraversalAction::SeedHistoryAndTraverseInWebContent:
        update_navigation_action_state();
        if (decision.outcome.will_replace_web_content_process)
            replace_web_content_process_for_top_level_traversal();
        dump_session_history("traverse-fallback-seed-history"sv);
        seed_web_content_session_history_from_ui_process();
        return decision.outcome;
    }
    VERIFY_NOT_REACHED();
...
JsonValue ViewImplementation::webdriver_session_history() const
{
    ...
    if (auto pending_step = m_top_level_traversable.pending_web_content_session_history_seed().step_to_traverse_after_seed; pending_step.has_value())
        serialized.set("pendingWebContentHistoryStepAfterSeed"sv, *pending_step);
    else
        serialized.set("pendingWebContentHistoryStepAfterSeed"sv, JsonValue {});
...
void ViewImplementation::seed_session_history_and_traverse_to_step_from_ui_process(TraversableSessionHistory::TraversalTarget const& target, StringView dump_reason)
{
    auto target_url = m_top_level_traversable.prepare_to_seed_session_history_and_traverse_to_step_from_ui_process(target, m_url);
    update_navigation_action_state();

    m_webdriver_pending_navigation_url = target_url;
    // NB: A UI-driven traversal is only fully observable once WebContent has accepted the UI-owned history seed and
    //     applied the target step. Completing WebDriver any earlier would let tests, and callers doing immediate
    //     history inspection, observe the traversal before the authoritative history has been consumed.
    m_webdriver_pending_navigation_completes_with_session_history_update = true;
    // NB: The process-swap decision was recorded on the pending traversal when the traversal was decided; consume
    //     that decision rather than re-deriving it from m_url, which can be stale while a traversal is in flight.
    auto will_replace_web_content_process = m_top_level_traversable.pending_session_history_traversal().has_value()
        && m_top_level_traversable.pending_session_history_traversal()->will_replace_web_content_process;
...
void ViewImplementation::did_traverse_the_history_to_step(Badge<WebContentClient>, i32 step, bool step_was_available, Web::HTML::HistoryStepResult result)
{
    ...
    if (step_result.fallback_target.has_value()) {
        seed_session_history_and_traverse_to_step_from_ui_process(*step_result.fallback_target, step_result.dump_reason);
        return;
    }
    ...
    Badge<WebContentClient>, u64 request_id, i32 step, bool canceled)
{
    ...
    if (check_result.target.has_value()) {
        ...
        seed_session_history_and_traverse_to_step_from_ui_process(*check_result.target, check_result.dump_reason);
        return;
    }
...
void ViewImplementation::handle_web_content_process_crash(LoadErrorPage load_error_page)
{
    ...
    auto crash_recovery = m_top_level_traversable.prepare_for_web_content_crash_recovery();
    ...
    if (load_error_page == LoadErrorPage::Yes) {
        ...
        m_should_suppress_history_for_current_load = false;
        m_should_suppress_history_for_next_load = false;
        // NB: Crash recovery is only observable once the replacement process has consumed the seeded history
        //     and applied the recovered step; WebDriver waits on that completion. A crash during a traversal
        //     keeps that traversal's pending navigation instead.
        if (!m_webdriver_pending_navigation_url.has_value()) {
            m_webdriver_pending_navigation_url = m_url;
            m_webdriver_pending_navigation_completes_with_session_history_update = true;
        }
