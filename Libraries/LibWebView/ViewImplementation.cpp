...
void ViewImplementation::reload()
{
    ...
    if (m_crash_state.has_value() && m_crash_state->navigation_to_retry.has_value()) {
        auto url = m_crash_state->navigation_to_retry.release_value();
        load(url);
        return;
    }

    if (m_crash_state.has_value() && !current_entry) {
        auto url = m_crash_state->failed_url;
        load(url);
        return;
    }
...
void ViewImplementation::handle_web_content_process_crash()
{
    ...
    // Only count crashes from the replacement page towards the loop limit. Separate, explicitly triggered crashes
    // may happen close together without indicating that restoring the page itself is crash-looping.
    ...
    auto const crashed_repeatedly = m_crash_count >= max_reasonable_crash_count;
    ...
    if (recovery_mode == RecoveryMode::ShowOverlay && crashed_repeatedly) {
        dbgln("WebContent has crashed {} times in quick succession! Waiting for an explicit navigation before restarting...", m_crash_count);
        if (auto queue_promise = move(m_pending_session_history_reset_queue_promise))
            queue_promise->resolve({});
        ...
        set_crash_state(CrashState {
            ...
            .needs_web_content_process_respawn = true,
        });
        for (auto command_id : pending_crash_command_ids)
...
void ViewImplementation::prepare_for_navigation_after_crash(Optional<URL::URL> navigation_to_retry)
{
    ...
    if (m_crash_state->needs_web_content_process_respawn) {
        m_crash_state->needs_web_content_process_respawn = false;
        m_crash_count = 0;
        respawn_web_content_process_after_crash();
    }
...
String ViewImplementation::crash_overlay_message() const
{
    auto serialized_url = m_crash_state.has_value() ? m_crash_state->failed_url.serialize() : m_url.serialize();
    constexpr size_t maximum_display_url_length = 256;
    auto display_url = serialized_url.code_points().length() > maximum_display_url_length
        ? MUST(String::formatted("{}…", serialized_url.code_points().unicode_substring_view(0, maximum_display_url_length)))
        : move(serialized_url);
    return MUST(String::formatted("The web page {} has crashed.\n\nYou can reload the page to try again.", display_url));
}
