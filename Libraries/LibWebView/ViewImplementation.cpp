...
void ViewImplementation::did_update_session_history(Badge<WebContentClient>, Vector<Web::HTML::SessionHistoryEntryDescriptor> entries, Vector<i32> used_steps, size_t current_used_step_index)
{
    ...
    case WebContentSessionHistoryUpdateStatus::IgnoredBeforeSeedAck:
        dump_session_history("ignored-session-history-before-ui-seed-ack"sv);
        update_navigation_action_state();
        return;
    case WebContentSessionHistoryUpdateStatus::IgnoredPartialBeforeFallbackSeed:
        dump_session_history("ignored-partial-session-history-before-fallback-seed"sv);
        update_navigation_action_state();
        return;
    case WebContentSessionHistoryUpdateStatus::IgnoredBeforeSeed:
        dump_session_history("ignored-session-history-before-ui-seed"sv);
        update_navigation_action_state();
        return;
    case WebContentSessionHistoryUpdateStatus::IgnoredRejectedSeedSnapshotForTesting:
        VERIFY_NOT_REACHED();
    case WebContentSessionHistoryUpdateStatus::Updated:
        break;
    }
...
void ViewImplementation::did_traverse_the_history_to_step(Badge<WebContentClient>, i32 step, bool step_was_available, Web::HTML::HistoryStepResult result)
{
    ...
    case WebContentHistoryStepStatus::IgnoredStale:
        dump_session_history("ignored-stale-webcontent-history-step-result"sv);
        return;
    case WebContentHistoryStepStatus::FallbackLoad:
        load_session_history_traversal_target_from_ui_process(*step_result.fallback_target,
            step_was_available ? "webcontent-history-step-applied-with-stale-mirror-fallback-load"sv : "webcontent-history-step-unavailable-fallback-load"sv);
        return;
    case WebContentHistoryStepStatus::Unavailable:
        update_navigation_action_state();
        dump_session_history(step_was_available ? "webcontent-history-step-applied-without-ui-target"sv : "webcontent-history-step-unavailable"sv);
        return;
    case WebContentHistoryStepStatus::Canceled:
        if (step_result.should_complete_webdriver_pending_navigation)
            complete_webdriver_pending_navigation_if_url_matches(m_url);
        update_navigation_action_state();
        dump_session_history("webcontent-history-step-canceled"sv);
        return;
    case WebContentHistoryStepStatus::Applied:
        if (step_result.should_complete_webdriver_pending_navigation)
            complete_webdriver_pending_navigation_if_url_matches(m_url);
        update_navigation_action_state();
        dump_session_history("webcontent-history-step-applied"sv);
        return;
    case WebContentHistoryStepStatus::Restored:
        if (step_result.should_complete_webdriver_pending_navigation)
            complete_webdriver_pending_navigation_if_url_matches(m_url);
        update_navigation_action_state();
        dump_session_history("webcontent-history-step-restored"sv);
        return;
    case WebContentHistoryStepStatus::RestorePendingNavigation: {
        auto reason = step_result.pending_step_was_available ? "webcontent-pending-history-step-canceled"sv : "webcontent-history-step-unavailable"sv;
        if (restore_pending_session_history_navigation(reason))
            return;
        VERIFY_NOT_REACHED();
    }
    case WebContentHistoryStepStatus::PendingStepFailed: {
        auto reason = step_result.pending_step_was_available ? "webcontent-pending-history-step-canceled"sv : "webcontent-history-step-unavailable"sv;
        update_navigation_action_state();
        dump_session_history(reason);
        return;
    ...
    Badge<WebContentClient>, u64 request_id, i32 step, bool canceled)
{
    ...
    case HistoryStepCancelationCheckStatus::IgnoredStale:
        dump_session_history("ignored-stale-history-step-cancelation-check-result"sv);
        return;
    case HistoryStepCancelationCheckStatus::Canceled:
        complete_webdriver_pending_navigation_if_url_matches(m_url);
        update_navigation_action_state();
        ...
        return;
    case HistoryStepCancelationCheckStatus::NoTarget:
        update_navigation_action_state();
        dump_session_history("traverse-fallback-cancelation-check-without-ui-target"sv);
        if (check_result.on_cancelation_check_complete)
            check_result.on_cancelation_check_complete(move(check_result.outcome));
        return;
    case HistoryStepCancelationCheckStatus::LoadTarget:
        if (check_result.on_cancelation_check_complete)
            check_result.on_cancelation_check_complete(move(check_result.outcome));
        load_session_history_traversal_target_from_ui_process(*check_result.target, "traverse-fallback-load-after-cancelation-check"sv);
        return;
    }
    VERIFY_NOT_REACHED();
}
