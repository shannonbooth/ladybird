...
public:
    ...
    RefPtr<CanonicalDocument> active_document() const;
...
private:
    ...
    // NB: A Document holds its browsing context. The active document is held by the navigable it is active in, or by
    //     the navigation that creates it, and is gone otherwise: a browsing context from a browsing context group
    //     switch is created with a document that is never used.
