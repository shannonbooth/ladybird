...
// NB: The specification finds the top-level browsing context through the node navigables of active documents, which
//     reaches the displayed document's while the document containing a browsing context's embedder is still to be
//     activated. The embedder's node document's browsing context never changes, so it is kept from creation.
