...
void CanonicalNavigable::discard_embedded_page(NonnullRefPtr<WebContentClient> client, u64 page_id)
{
...
bool CanonicalNavigable::pending_host_matches(WebContentClient const& client, u64 page_id) const
{
...
void CanonicalNavigable::set_pending_host(NonnullRefPtr<WebContentClient> client, u64 page_id)
{
...
void CanonicalNavigable::clear_ongoing_navigation()
{
    ...
    // A host chosen for the navigation's document takes the container over before the navigation is cleared, so a
    // host still pending here was chosen for a document that is not coming.
    discard_pending_host();
}
