...
NonnullRefPtr<CanonicalDocument> CanonicalNavigable::create_and_initialize_a_document(NavigationLoader::ResponseDocument const& navigation_params)
{
    ...
    // 7. Otherwise:
        ...
        // NB: Only a top-level browsing context has a group. A child browsing context's is its top-level browsing
        //     context's.
...
void CanonicalNavigable::discard_pending_host()
{
    ...
    // The chosen page drops the provisional navigable it created for a document another page displays.
...
void CanonicalNavigable::did_commit_navigation(CanonicalSessionHistoryEntry const& entry, Web::HTML::ReplicatedNavigableState replicated_state, Optional<Utf16String> const& navigation_id, DidPopulateDocument did_populate_document, RefPtr<WebContentPage> host)
{
    ...
    replicated_state.active_session_history_entry_identity = entry.identity();
