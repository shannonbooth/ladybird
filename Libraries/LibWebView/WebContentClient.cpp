...
void WebContentClient::did_request_new_process_for_child_frame_navigation(u64 page_id, String frame_id, URL::URL url, Variant<Empty, String, Web::HTML::POSTResource> document_resource, Web::Bindings::NavigationHistoryBehavior history_handling)
{
    ...
    auto child_navigable = site_isolation_manager.child_navigable(page_id, frame_id);
    if (!child_navigable.has_value())
        return;
    if (!site_isolation_manager.has_matching_pending_child_frame_navigation(page_id, frame_id, url, CanonicalNavigable::PendingNavigationHost::Remote))
        return;
    ...
    site_isolation_manager.record_pending_child_frame_navigation(page_id, frame_id, url, CanonicalNavigable::PendingNavigationHost::Remote);
    ...
    if (child_navigable->viewport_rect().has_value()) {
        remote_client->async_set_viewport(
            remote_page_id,
            child_navigable->viewport_rect()->size(),
            child_navigable->device_pixel_ratio(),
...
void WebContentClient::did_request_webdriver_history_traversal(u64 page_id, u64 request_id, i32 delta)
{
    if (auto view = view_for_page_id(page_id); view.has_value()) {
        ...
        Core::deferred_invoke([this, page_id, request_id, view_id, delta] {
            ...
            auto complete = [this, page_id, request_id](HistoryTraversalOutcome outcome) {
            ...
            auto outcome = view->traverse_the_history_by_delta(delta, CheckForCancelation::Yes,
                [this, page_id, request_id](HistoryTraversalOutcome outcome) {
                    auto traversal_started = outcome.status == HistoryTraversalStatus::Started;
                    async_complete_webdriver_history_traversal(
                        page_id,
                        request_id,
