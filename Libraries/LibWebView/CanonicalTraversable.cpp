...
// A synchronous same-document navigation snapshots its new entry when requesting finalization, so an update sent
// immediately after the request can precede the canonical entry. Retry at the entry's queue position, behind the
// pending finalization; an entry still missing there means the finalization failed and the update is moot.
void CanonicalTraversable::retry_session_history_entry_update_at_queue_position(CanonicalNavigable const& navigable, Utf16String const& navigation_api_key, Function<void(TraversableSessionHistory::Entry&)> update_entry)
{
    m_history_traversal_queue.append_session_history_synchronous_navigation_steps(navigable.id(),
        [this, navigable_id = navigable.id(), navigation_api_key, update_entry = move(update_entry)](NonnullRefPtr<Core::Promise<Empty>> promise) mutable {
            if (auto navigable = find(navigable_id); navigable.has_value()) {
                if (m_session_history.update_entry(nested_history_id_for(*navigable), navigation_api_key, update_entry))
                    session_history_changed();
            }
            promise->resolve({});
        });
...
bool CanonicalTraversable::update_session_history_entry_navigation_api_state(CanonicalNavigable const& navigable, Utf16String const& navigation_api_key, Web::HTML::StorageSerializationRecord navigation_api_state)
{
    ...
    retry_session_history_entry_update_at_queue_position(navigable, navigation_api_key, [navigation_api_state = move(navigation_api_state)](auto& entry) {
        entry.navigation_api_state = navigation_api_state;
    });
    return false;
}
...
bool CanonicalTraversable::update_session_history_entry_scroll_restoration_mode(CanonicalNavigable const& navigable, Utf16String const& navigation_api_key, Web::HTML::ScrollRestorationMode scroll_restoration_mode)
{
    ...
    retry_session_history_entry_update_at_queue_position(navigable, navigation_api_key, [scroll_restoration_mode](auto& entry) {
        entry.scroll_restoration_mode = scroll_restoration_mode;
    });
    return false;
}
...
void CanonicalTraversable::traverse_the_history(TraversableSessionHistory::TraversalTarget const& target, CheckForCancelation check_for_cancelation, Function<void()> on_ready, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    run_browser_history_traversal_at_queue_position(
        Web::BrowserHistoryTraversalRequest {
            .traversable_id = id(),
...
struct CanonicalTraversable::HistoryOperation {
    HistoryOperation(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationRequest parameters, RefPtr<WebContentClient> initiating_client, u64 initiating_page_id, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete)
    ...
    Web::HistoryOperationRequest parameters;
...
Optional<size_t> CanonicalTraversable::effective_current_session_history_step_index() const
{
    ...
    for (auto const& operation : m_history_operations) {
        if (!operation.value->is_browser_traversal())
            continue;
        auto const& parameters = operation.value->parameters.get<Web::BrowserHistoryTraversalRequest>();
...
{
    // AD-HOC (browser UI): The HTML algorithms do not prescribe how repeated Back/Forward button presses are
    // coalesced while an earlier traversal is queued. Resolve the combined target at the queue position so rapid
    // input observes the same history mutations as separate traversals without starting redundant unload work.
    if (m_pending_browser_history_traversal.has_value()) {
...
{
    auto const& parameters = operation.parameters.get<Web::BrowserHistoryTraversalRequest>();
...
Optional<CanonicalTraversable::BrowserHistoryTraversalDiagnostic> CanonicalTraversable::browser_history_traversal_for_testing() const
{
    for (auto const& operation : m_history_operations) {
        if (!operation.value->is_browser_traversal())
            continue;
        VERIFY(operation.value->parameters.has<Web::BrowserHistoryTraversalRequest>());
        auto const& parameters = operation.value->parameters.get<Web::BrowserHistoryTraversalRequest>();
        auto target = m_session_history.traversal_target_for_step(parameters.target_step);
        if (!target.has_value())
            return {};
...
{
    // AD-HOC (crash recovery): The HTML Standard assumes its participating agents remain alive. Rebind or finish
    // the jobs owned by a crashed WebContent endpoint, then reconstruct the current canonical entry in its replacement.
    for (auto& operation : m_history_operations) {
        if (!operation.value->is_browser_traversal())
            continue;
...
{
    if (!operation.parameters.has<Web::FinalizeCrossDocumentNavigationRequest>())
        return true;

    auto const& parameters = operation.parameters.get<Web::FinalizeCrossDocumentNavigationRequest>();
...
bool CanonicalTraversable::select_changing_navigable_history_step_job_endpoint(HistoryOperation& operation, ApplyHistoryStepJobs::ChangingNavigableHistoryStepJob& job)
{
    ...
    if (navigable->is_top_level_traversable()
        && operation.parameters.has<Web::ReloadRequest>()) {
        if (auto view = ViewImplementation::find_view_for_traversable(*this); view.has_value())
...
void CanonicalTraversable::run_history_operation_at_queue_position(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationRequest request, WebContentClient* requesting_client, u64 requesting_page_id, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete, NonnullRefPtr<Core::Promise<Empty>> promise)
{
...
void CanonicalTraversable::run_browser_history_traversal_at_queue_position(Web::BrowserHistoryTraversalRequest parameters, bool check_for_cancelation, u64 traversal_sequence_number, Function<void()> on_ready, OnHistoryOperationComplete on_complete, NonnullRefPtr<Core::Promise<Empty>> promise)
{
    auto operation_id = Application::the().allocate_ui_process_cross_process_id();
    auto owned_operation = make<HistoryOperation>(operation_id, Web::HistoryOperationRequest { move(parameters) }, nullptr, 0, traversal_sequence_number, move(on_complete));
    ...
    if (m_pending_browser_history_traversal.has_value()
        && m_pending_browser_history_traversal->stage == PendingBrowserHistoryTraversal::Stage::Running
        && m_pending_browser_history_traversal->target_step == operation->parameters.get<Web::BrowserHistoryTraversalRequest>().target_step
...
void CanonicalTraversable::enqueue_history_operation(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationRequest request, WebContentClient& requesting_client, u64 requesting_page_id, Optional<u64> traversal_sequence_number, OnHistoryOperationComplete on_complete)
{
    ...
    if (request.has<Web::DestroyChildNavigableRequest>()) {
        auto const& parameters = request.get<Web::DestroyChildNavigableRequest>();
    ...
    if (request.has<Web::FinalizeSameDocumentNavigationRequest>())
        synchronous_navigation_target = request.get<Web::FinalizeSameDocumentNavigationRequest>().navigable_id;
...
void CanonicalTraversable::enqueue_browser_history_traversal(Web::BrowserHistoryTraversalRequest parameters, bool check_for_cancelation, OnHistoryOperationComplete on_complete)
{
...
void CanonicalTraversable::apply_the_history_step(HistoryOperation& operation, i32 step, bool check_for_cancelation, Optional<Web::SerializedSourceSnapshotParams> source_snapshot_params, Optional<Web::HTML::CrossProcessId> initiator_to_check, Web::HTML::UserNavigationInvolvement user_involvement, Optional<Web::Bindings::NavigationType> navigation_type)
{
    ...
    operation.algorithm = make<ApplyHistoryStep>(
        ...
        step, check_for_cancelation, initiator_to_check, source_snapshot_params, user_involvement, navigation_type,
...
void CanonicalTraversable::apply_the_push_or_replace_history_step(HistoryOperation& operation, i32 step, Web::HTML::HistoryHandlingBehavior history_handling, Web::HTML::UserNavigationInvolvement user_involvement)
{
    ...
    apply_the_history_step(operation, step, false, {}, {}, user_involvement, navigation_type);
}
...
void CanonicalTraversable::apply_the_reload_history_step(HistoryOperation& operation, Web::HTML::UserNavigationInvolvement user_involvement)
{
    ...
    apply_the_history_step(operation, *step, true, {}, {}, user_involvement, Web::Bindings::NavigationType::Reload);
}

// https://html.spec.whatwg.org/multipage/browsing-the-web.html#apply-the-traverse-history-step
void CanonicalTraversable::apply_the_traverse_history_step(HistoryOperation& operation, i32 step, Optional<Web::SerializedSourceSnapshotParams> source_snapshot_params, Optional<Web::HTML::CrossProcessId> initiator_to_check, Web::HTML::UserNavigationInvolvement user_involvement)
{
    ...
    apply_the_history_step(operation, step, true, move(source_snapshot_params), initiator_to_check, user_involvement, Web::Bindings::NavigationType::Traverse);
}
...
void CanonicalTraversable::resume_applying_the_traverse_history_step(HistoryOperation& operation, i32 step, Web::HTML::UserNavigationInvolvement user_involvement)
{
    ...
    apply_the_history_step(operation, step, false, {}, {}, user_involvement, Web::Bindings::NavigationType::Traverse);
}
...
void CanonicalTraversable::update_for_navigable_creation_or_destruction(HistoryOperation& operation)
{
    ...
    apply_the_history_step(operation, *step, false, {}, {}, Web::HTML::UserNavigationInvolvement::None, {});
}

// These requests have their complete input in canonical state, so they run at their queue position without a
// WebContent preparation round trip.
static bool history_operation_request_runs_without_web_content_preparation(Web::HistoryOperationRequest const& parameters)
{
    return parameters.has<Web::ReloadRequest>()
        || parameters.has<Web::TraverseHistoryByADeltaRequest>()
        || parameters.has<Web::BrowserHistoryTraversalRequest>()
        || parameters.has<Web::PerformNavigationAPITraversalRequest>()
        || parameters.has<Web::FinalizeSameDocumentNavigationRequest>()
        || parameters.has<Web::DestroyChildNavigableRequest>()
        || parameters.has<Web::DefinitelyCloseTopLevelTraversableRequest>()
        || parameters.has<Web::FlushSessionHistoryTraversalQueueForTestingRequest>();
}

// https://html.spec.whatwg.org/multipage/browsing-the-web.html#traverse-the-history-by-a-delta
void CanonicalTraversable::traverse_the_history_by_a_delta_at_queue_position(HistoryOperation& operation, Web::TraverseHistoryByADeltaRequest const& request)
{
    ...
    apply_the_traverse_history_step(operation, all_steps[static_cast<size_t>(target_step_index)], request.source_snapshot_params, request.initiator_to_check, request.user_involvement);
}

// https://html.spec.whatwg.org/multipage/nav-history-apis.html#performing-a-navigation-api-traversal
void CanonicalTraversable::perform_a_navigation_api_traversal_at_queue_position(HistoryOperation& operation, Web::PerformNavigationAPITraversalRequest const& request)
{
    ...
    apply_the_traverse_history_step(
        operation,
        target_entry->step,
        request.source_snapshot_params,
        request.navigable_id,
        Web::HTML::UserNavigationInvolvement::None);
...
void CanonicalTraversable::finalize_a_same_document_navigation(HistoryOperation& operation, Web::FinalizeSameDocumentNavigationRequest const& request)
{
    ...
    // NB: This persisted state crosses the process boundary separately from targetEntry. Apply it at the same queue
    // position before rebuilding targetEntry from canonical document state.
    if (request.previous_entry_persisted_state.has_value()) {
        m_session_history.update_entry_persisted_state(
            nested_history_id_for(*target_navigable),
            *request.previous_entry_persisted_state);
    }
    ...
    auto target_document_state = target_entries->find_if([&](auto const& entry) {
        return entry.document_state.id == request.target_entry.document_state_id;
    });
    ...
    auto canonical_target_entry = TraversableSessionHistory::Entry {
        .step = 0,
        .url = request.target_entry.url,
        .document_state = target_document_state->document_state,
        .classic_history_api_state = request.target_entry.classic_history_api_state,
        .navigation_api_state = request.target_entry.navigation_api_state,
        .navigation_api_key = request.target_entry.navigation_api_key,
        .navigation_api_id = request.target_entry.navigation_api_id,
        .scroll_restoration_mode = request.target_entry.scroll_restoration_mode,
        .scroll_position_data = request.target_entry.scroll_position_data,
    };
    ...
    target_navigable->set_active_session_history_entry_identity(Web::HTML::SessionHistoryEntryIdentity {
        .document_state_id = request.target_entry.document_state_id,
        .navigation_api_id = request.target_entry.navigation_api_id,
    });
...
void CanonicalTraversable::run_history_operation_request_at_queue_position(HistoryOperation& operation)
{
    operation.parameters.visit(
        [&](Web::ReloadRequest const& request) {
            auto current_step = m_session_history.current_step();
            auto navigable = find(request.navigable_id);
            ...
            apply_the_reload_history_step(operation, request.user_involvement);
        },
        [&](Web::TraverseHistoryByADeltaRequest const& request) {
            traverse_the_history_by_a_delta_at_queue_position(operation, request);
        },
        [&](Web::BrowserHistoryTraversalRequest const& request) {
            apply_the_traverse_history_step(operation, request.target_step, {}, {}, request.user_involvement);
        },
        [&](Web::PerformNavigationAPITraversalRequest const& request) {
            perform_a_navigation_api_traversal_at_queue_position(operation, request);
        },
        [&](Web::FinalizeSameDocumentNavigationRequest const& request) {
            finalize_a_same_document_navigation(operation, request);
        },
        [&](Web::DestroyChildNavigableRequest const&) {
            update_for_navigable_creation_or_destruction(operation);
        },
        [&](Web::DefinitelyCloseTopLevelTraversableRequest const&) {
        ...
        [&](Web::FlushSessionHistoryTraversalQueueForTestingRequest const&) {
...
void CanonicalTraversable::start_history_operation(HistoryOperation& operation, NonnullRefPtr<Core::Promise<Empty>>)
{
    ...
    if (operation.is_browser_traversal()) {
        ...
        VERIFY(operation.parameters.has<Web::BrowserHistoryTraversalRequest>());
        auto const& parameters = operation.parameters.get<Web::BrowserHistoryTraversalRequest>();
        // AD-HOC (browser UI): Apply targetStep given the embedder's checkForCancelation choice, null sourceSnapshotParams, null initiatorToCheck, userInvolvement, and "traverse".
        apply_the_history_step(operation, parameters.target_step, operation.check_for_cancelation, {}, {}, parameters.user_involvement, Web::Bindings::NavigationType::Traverse);
        return;
    }
    ...
    if (history_operation_request_runs_without_web_content_preparation(operation.parameters)) {
        run_history_operation_request_at_queue_position(operation);
        return;
    }
    ...
    if (operation.parameters.has<Web::CreateNewChildNavigableRequest>()) {
        auto const& parameters = operation.parameters.get<Web::CreateNewChildNavigableRequest>();
...
void CanonicalTraversable::did_receive_history_operation_ready(WebContentClient& source_client, u64 source_page_id, Web::HTML::CrossProcessId operation_id, Web::HistoryOperationReadyResult result)
{
    ...
    auto result_matches_request = request.visit(
        [&](Web::FinalizeCrossDocumentNavigationRequest const&) { return result.has<Web::CrossDocumentNavigationFinalizationHostState>(); },
        [&](Web::CreateNewChildNavigableRequest const&) { return result.has<Web::HTML::CrossProcessId>(); },
        [&](Web::FinalizeSameDocumentNavigationRequest const&) { return false; },
        [&](Web::DefinitelyCloseTopLevelTraversableRequest const&) { return false; },
        [&](Web::FlushSessionHistoryTraversalQueueForTestingRequest const&) { return false; },
    ...
    request.visit(
        [&](Web::FinalizeCrossDocumentNavigationRequest const&) {
        ...
        [&](Web::ReloadRequest const&) {
            VERIFY_NOT_REACHED();
        },
        [&](Web::TraverseHistoryByADeltaRequest const&) {
            VERIFY_NOT_REACHED();
        },
        [&](Web::BrowserHistoryTraversalRequest const&) {
            VERIFY_NOT_REACHED();
        },
        [&](Web::PerformNavigationAPITraversalRequest const&) {
            VERIFY_NOT_REACHED();
        },
        [&](Web::ResumeTraverseHistoryStepRequest const& parameters) {
            resume_applying_the_traverse_history_step(*operation, parameters.target_step, parameters.user_involvement);
        },
        [&](Web::CreateNewChildNavigableRequest const& parameters) {
        ...
        [&](Web::FinalizeSameDocumentNavigationRequest const&) {
            VERIFY_NOT_REACHED();
        },
        [&](Web::DefinitelyCloseTopLevelTraversableRequest const&) {
...
void CanonicalTraversable::finish_history_operation(Web::HTML::CrossProcessId operation_id, Web::HTML::HistoryStepResult result, Optional<i32> committed_step)
{
    ...
    if (taken_operation.owns_navigation_transaction) {
        auto const& parameters = taken_operation.parameters.get<Web::FinalizeCrossDocumentNavigationRequest>();
        ...
        if (activated_navigable_state.has_value()) {
            ...
            if (navigable.has_value()) {
                auto navigation_id = operation->parameters.visit(
                    [](Web::FinalizeCrossDocumentNavigationRequest const& parameters) { return parameters.navigation_id; },
