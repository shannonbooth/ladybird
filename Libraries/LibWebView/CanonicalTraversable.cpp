...
struct CanonicalTraversable::HistoryOperation {
    ...
    // The UI-process admission order of this traversal relative to navigation starts.
    // This is deliberately distinct from operation_id, which is allocated only once
    // the operation reaches its session-history queue position.
...
bool CanonicalTraversable::navigation_transaction_matches(HistoryOperation const& operation, WebContentClient const& client, u64 page_id, Optional<Web::HTML::CrossProcessId> reply_navigable_id) const
{
    ...
    return navigable.has_value()
        && navigable->navigation_transaction_matches(parameters.navigation_id, client, page_id);
}
    ...
    if (job.navigation_type == Web::Bindings::NavigationType::Traverse
        ...
        if (auto& ongoing_navigation = navigable->ongoing_navigation(); ongoing_navigation.has_value()) {
            if (ongoing_navigation->sequence_number > *operation.traversal_sequence_number) {
                // This navigation was admitted by the canonical coordinator after the traversal.
                // Let it own the visible outcome instead of asking WebContent to infer precedence
                // from the mere presence of a local navigation ID.
                job.superseded_by_newer_navigation = true;
            } else {
                // Replacing the canonical transaction makes all late population/finalization
                // callbacks for the older navigation fail their navigation-token checks.
                navigable->clear_ongoing_navigation();
            }
        }
    }
    ...
    if (operation.parameters.has<Web::NavigableCreationHistoryOperationParameters>()) {
        ...
        if (child_navigable.has_value() && current_step.has_value()) {
            if (auto const* target_entry = m_session_history.get_the_target_history_entry(*child_navigable, *current_step)) {
                // AD-HOC: The UI process routes a child created during history reconstruction to this canonical
                // entry. The live child keeps its initial about:blank active while it populates a pending entry from
                // this descriptor, but the canonical history is already positioned at the traversal target.
...
{
    VERIFY(operation.parameters.has<Web::FinalizeCrossDocumentNavigationHistoryOperationParameters>());
    auto const& parameters = operation.parameters.get<Web::FinalizeCrossDocumentNavigationHistoryOperationParameters>();
    auto navigable = find(parameters.navigable_id);
    if (!navigable.has_value()) {
        finish_history_operation(operation.operation_id, Web::HTML::HistoryStepResult::NoMatchingEntry, {});
