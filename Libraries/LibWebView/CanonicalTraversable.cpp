...
CanonicalTraversable& CanonicalTraversable::create_a_new_top_level_traversable(Web::HTML::CrossProcessId id, Optional<CanonicalNavigable&> opener, Web::HTML::SessionHistoryEntryDescriptor initial_history_entry, WebContentClient& process)
{
    ...
    auto history_entry = CanonicalSessionHistoryEntry::create_from_descriptor(initial_history_entry, document_states);
...
NonnullRefPtr<CanonicalSessionHistoryEntry> CanonicalTraversable::session_history_entry_for(Web::HTML::SameDocumentNavigationEntry const& same_document_entry) const
{
    auto document_state = m_session_history.find_document_state(same_document_entry.document_state_id);
...
void CanonicalTraversable::enqueue_history_operation(Web::HTML::CrossProcessId operation_id, Web::HistoryOperationParameters request, RefPtr<WebContentPage> requesting_page, u64 sequence_number, OnHistoryOperationComplete on_complete)
{
    ...
    if (request.has<Web::FinalizeSameDocumentNavigationHistoryOperationParameters>()) {
        ...
        if (target_navigable.has_value() && &target_navigable->top_level_traversable() == this) {
            ...
            target_navigable->stage_same_document_session_history_entry(operation_id, session_history_entry_for(parameters.target_entry));
            session_history_changed();
        }
    }
    ...
    if (request.has<Web::NavigableCreationHistoryOperationParameters>()) {
        ...
        CanonicalSessionHistoryEntry::DocumentStates document_states;
        created_navigable_history_entry = CanonicalSessionHistoryEntry::create_from_descriptor(Web::HTML::create_session_history_entry_descriptor(parameters.initial_history_entry, 0), document_states);
        if (auto navigable = find(parameters.navigable_id); navigable.has_value()) {
            // A navigable created while the history is reconstructed keeps the current entry it was given.
            if (!navigable->current_session_history_entry())
