...
CanonicalNavigable& CanonicalTraversable::insert(NonnullRefPtr<WebContentPage> reporting_page, Web::HTML::CrossProcessId parent_frame_id, Web::HTML::CrossProcessId frame_id, Web::HTML::ReplicatedNavigableState replicated_state, NonnullRefPtr<CanonicalDocument> document, CanonicalNavigable& fallback_parent)
{
    ...
    // The container is in the parent's document the reporting page hosts.
    auto container_document = parent->document_hosted_by(reporting_page);
    if (!container_document)
        container_document = parent->active_document();

    auto navigable = make<CanonicalNavigable>(frame_id, parent_frame_id, move(container_document));
...
void CanonicalTraversable::for_each_hosting_page(Function<void(WebContentPage&)> const& callback) const
{
    ...
    for_each_in_subtree([&](CanonicalNavigable const& navigable) {
        ...
        if (auto pending_host = navigable.pending_host())
            visit(*pending_host);
        return IterationDecision::Continue;
    });
...
// The view's page, while it populates the traversable's next document rather than hosting the active one.
RefPtr<WebContentPage> CanonicalTraversable::pending_host() const
{
    auto page = display_page();
    if (!page || !has_active_document() || active_document().host() == page)
        return nullptr;
    return page;
}
...
void CanonicalTraversable::discard_stand_in(NonnullRefPtr<WebContentPage> page)
{
    if (!page->is_live())
        return;
    ...
    if (!page->displays_tab() && !is_representing_page(page) && page->client().holds_part_of_a_tab_related_to(*this))
        m_representing_pages.append(page);
    release_page_if_unused(move(page));
}

// The page still hosting the traversable's document while another page displays the tab, until the document is
// unloaded.
RefPtr<WebContentPage> CanonicalTraversable::displaced_document_host() const
{
    ...
    if (!host || host == display_page())
        return nullptr;
    return host;
}

// The displaced document's host, until the document is being unloaded.
RefPtr<WebContentPage> CanonicalTraversable::page_displaying_displaced_document() const
{
    auto host = displaced_document_host();
    if (!host || !host->is_open() || active_document().unload_started())
        return nullptr;
    return host;
}

// The navigation or history step that was to replace the traversable's document ended without activating one. The
// document stays active, so the view returns to the page displaying it, unless it is being unloaded there already.
void CanonicalTraversable::return_to_displaced_document()
{
    auto host = displaced_document_host();
    if (!host)
        return;
    if (!host->is_live()) {
        active_document().set_unloaded();
        return;
    }
    if (active_document().unload_started())
        return;
...
void CanonicalTraversable::release_displaced_document_host_after_unload()
{
    ...
    represent_in_page_or_release_it(*this, host.release_nonnull());
}
...
void CanonicalTraversable::create_a_new_top_level_traversable(Optional<CanonicalNavigable&> opener, Web::HTML::SessionHistoryEntryDescriptor initial_history_entry, WebContentPage& page)
{
    ...
    if (!opener.has_value())
        set_active_document(CanonicalBrowsingContext::create_a_new_top_level_browsing_context_and_document(document_origin, page).document);
    ...
    // 3. Otherwise, set document to the second return value of creating a new auxiliary browsing context and document given opener.
        ...
        set_active_document(CanonicalBrowsingContext::create_a_new_auxiliary_browsing_context_and_document(*opener, document_origin, page).document);
    ...
    Web::HTML::SessionHistoryEntryDescriptor active_entry)
{
    ...
    // The process displaying the tab installed its own active entry as the canonical one, with a document it created.
    if (auto page = display_page()) {
        auto document = document_created_by_host(active_entry.document_state.origin.value_or(URL::Origin::create_opaque()));
...
struct CanonicalTraversable::HistoryOperation {
    ...
    struct PendingChangingJob {
        ...
        // https://html.spec.whatwg.org/multipage/browsing-the-web.html#changing-navigable-continuation-displayed-document
        RefPtr<CanonicalDocument> displayed_document;
    };
...
RefPtr<CanonicalDocument> CanonicalTraversable::document_of_pending_history_job(CanonicalNavigable const& navigable) const
{
    for (auto const& operation : m_history_operations) {
        if (auto job = operation.value->pending_changing_jobs.get(navigable.id()); job.has_value() && job.value()->document)
...
RefPtr<WebContentPage> CanonicalTraversable::changing_job_endpoint(HistoryOperation const& operation, Web::HTML::CrossProcessId navigable_id) const
{
    ...
    return changing_job_endpoint(operation, navigable_id, job.has_value() ? job.value()->document : nullptr);
}

// The page a changing job runs in: the one hosting the document the job populated, or the document of the navigation
// it finalizes, which takes the container over once the job activates it, or the navigable's document until then.
RefPtr<WebContentPage> CanonicalTraversable::changing_job_endpoint(HistoryOperation const& operation, Web::HTML::CrossProcessId navigable_id, RefPtr<CanonicalDocument> const& populated_document) const
{
    if (populated_document)
        return populated_document->host();
    if (operation.parameters.has<Web::FinalizeCrossDocumentNavigationHistoryOperationParameters>()) {
        ...
        if (parameters.navigable_id == navigable_id && operation.destination_document)
            return operation.destination_document->host();
    }
...
CanonicalBrowsingContext& CanonicalTraversable::browsing_context_for_document_creation(WebContentPage const& page) const
{
    ...
    if (auto document = document_hosted_by(page))
...
void CanonicalTraversable::recover_from_web_content_process_crash(RefPtr<WebContentPage> crashed_endpoint, OnHistoryOperationComplete on_complete)
{
    for (auto& operation : m_history_operations) {
        ...
        // A job went with the crashed page when the document it populated was there, or when it was the traversable's
        // and had populated none: that job ran in the view's page.
        Vector<Web::HTML::CrossProcessId> crashed_changing_jobs;
        for (auto const& job : operation.value->pending_changing_jobs) {
            auto endpoint = changing_job_endpoint(*operation.value, job.key);
            if (!endpoint || !endpoint->is_live() || endpoint_crashed(*endpoint) || (job.key == id() && !job.value->document))
                crashed_changing_jobs.append(job.key);
        }
    ...
    for (auto& operation_entry : m_history_operations) {
        ...
        if (!lost_endpoint_is_root) {
            ...
            for (auto const& job : operation.pending_changing_jobs) {
                if (auto endpoint = changing_job_endpoint(operation, job.key); endpoint && endpoint_matches(*endpoint))
...
bool CanonicalTraversable::select_changing_navigable_history_step_job_endpoint(HistoryOperation& operation, ApplyHistoryStepJobs::ChangingNavigableHistoryStepJob& job)
{
    ...
    auto endpoint = changing_job_endpoint(operation, job.navigable_id, nullptr);
    ...
    if (response_document.has_value()) {
        ...
        if (!response_document->is_inline_content) {
            ...
            if (navigable->is_top_level_traversable() && (!process || process != &display_page()->client())) {
                ...
                auto host = navigable->obtain_page_to_host_document_in(*document, process);
                if (host.is_error()) {
                    // The job stays where it ran, without the document nothing hosts.
        ...
        // The document is created in the page displaying the tab, or the host chosen for it, and the job goes on there.
        if (!document->host())
            document->set_host(*page_hosting(*navigable));
        endpoint = document->host();
    }
    add_history_operation_completion_endpoint(*operation, *endpoint);
...
void CanonicalTraversable::unload_displayed_document_for_cross_document_navigation(HistoryOperation& operation, Web::HTML::CrossProcessId navigable_id)
{
    ...
    if (!endpoint)
        return;
    auto pending_job = operation.pending_changing_jobs.get(navigable_id);
    VERIFY(pending_job.has_value() && pending_job.value()->displayed_document);
    auto operation_id = operation.operation_id;
    unload_a_document_and_its_descendants(*pending_job.value()->displayed_document, navigable_id, *endpoint, Web::HTML::ChildNavigableDestruction::No, [this, operation_id, navigable_id](UnloadedInItsHost) {
...
void CanonicalTraversable::unload_a_document_and_its_descendants(CanonicalDocument& document, Web::HTML::CrossProcessId navigable_id, RefPtr<WebContentPage> continuing_endpoint, Web::HTML::ChildNavigableDestruction child_navigable_destruction, Function<void(UnloadedInItsHost)> queue_document_unload_task)
{
    ...
    document.set_unload_started();
    // The document is unloaded in the page hosting it, before the document replacing it activates, unless that is
    // the page continuing the invoking algorithm, which unloads it itself.
    RefPtr<WebContentPage> document_host = document.host();
    if (auto navigable = find(navigable_id); navigable.has_value()) {
        if (!document_host && navigable->parent())
    ...
    pending_unload.queue_document_unload_task = [this, navigable_id, continuing_endpoint, child_navigable_destruction, document_host = move(document_host), queue_document_unload_task = move(queue_document_unload_task)] mutable {
        ...
        unload_document_in_its_host(document_host.release_nonnull(), navigable_id, child_navigable_destruction, [this, navigable_id, host, queue_document_unload_task = move(queue_document_unload_task)] {
...
void CanonicalTraversable::did_receive_changing_navigable_unload_preparation_complete(WebContentPage& source_page, Web::HTML::CrossProcessId operation_id, Web::HTML::CrossProcessId navigable_id)
{
    ...
    if (!pending_job.has_value() || !pending_job.value()->unload_preparation_pending)
        return;
    if (changing_job_endpoint(*operation, navigable_id) != source_page)
        return;
...
void CanonicalTraversable::discard_pending_host_at(Web::HTML::CrossProcessId navigable_id, RefPtr<CanonicalDocument> document)
{
    auto navigable = find(navigable_id);
    if (navigable_id != id() && navigable.has_value() && document && document->host())
        navigable->discard_document(*document);
}
...
void CanonicalTraversable::did_receive_child_navigable_unload_request(WebContentPage& source_page, Web::HTML::CrossProcessId navigable_id)
{
    ...
    unload_a_document_and_its_descendants(navigable->active_document(), navigable_id, source_page, Web::HTML::ChildNavigableDestruction::Yes, [source_page = NonnullRefPtr<WebContentPage>(source_page), navigable_id](UnloadedInItsHost) {
        source_page->async_continue_child_navigable_destruction(navigable_id);
    });
}
...
void CanonicalTraversable::dispatch_crash_recovery_changing_job(HistoryOperation& operation, NonnullRefPtr<WebContentPage> endpoint, Web::HTML::HistoryObjectLengthAndIndex history_object_length_and_index, Function<void()> on_complete)
{
    ...
    if (has_active_document())
        pending_job->displayed_document = active_document();
...
void CanonicalTraversable::complete_history_jobs_after_crash(HistoryOperation& operation, Vector<Web::HTML::CrossProcessId> changing_jobs, Vector<Web::HTML::CrossProcessId> nonchanging_updates)
{
    ...
    for (auto navigable_id : changing_jobs) {
        ...
        if (!pending_job.has_value())
            continue;
        discard_pending_host_at(navigable_id, pending_job.value()->document);
...
ApplyHistoryStepJobs CanonicalTraversable::create_apply_history_step_jobs(Web::HTML::CrossProcessId operation_id)
{
    return {
        ...
        .run_changing_navigable_history_step_job = [this, operation_id](ApplyHistoryStepJobs::ChangingNavigableHistoryStepJob job, Function<void(Web::HTML::ChangingNavigableHistoryStepJobDisposition)> on_complete) {
            ...
            // 3. Let displayedDocument be navigable's active document.
            if (auto navigable = find(navigable_id); navigable.has_value() && navigable->has_active_document())
                pending_job->displayed_document = navigable->active_document();
...
void CanonicalTraversable::run_direct_history_operation(HistoryOperation& operation)
{
    operation.parameters.visit(
        ...
        [&](Web::CloseTopLevelTraversableHistoryOperationParameters const&) {
            ...
            unload_a_document_and_its_descendants(active_document(), id(), page_hosting(*this), Web::HTML::ChildNavigableDestruction::No, [this, operation_id = operation.operation_id](UnloadedInItsHost) {
                auto* operation = find_history_operation(operation_id);
                if (!operation)
                    return;
...
void CanonicalTraversable::finish_history_operation(Web::HTML::CrossProcessId operation_id, Web::HTML::HistoryStepResult result, Optional<i32> committed_step)
{
    ...
    for (auto const& job : taken_operation.pending_changing_jobs)
        discard_pending_host_at(job.key, job.value->document);
...
void CanonicalTraversable::dispatch_next_beforeunload_group(HistoryOperation& operation)
{
    while (!operation.pending_beforeunload_groups.is_empty()) {
        ...
        auto endpoint_is_available = group.endpoint->is_live();
