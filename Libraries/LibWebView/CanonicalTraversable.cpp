...
// 5.3. Unload a document and its descendants given displayedDocument, targetEntry's document, afterPotentialUnloads,
//      and firePageSwapBeforeUnload.
// NB: The process running the job applies step 5.2 to its projection of the navigable and reports back before this
//     runs.
...
void CanonicalTraversable::did_activate_history_entry(HistoryOperation& operation, Web::HTML::CrossProcessId navigable_id, WebContentClient& source_client, u64 source_page_id, Web::HTML::SessionHistoryEntryDescriptor const& target_entry, RefPtr<CanonicalBrowsingContext> destination_browsing_context, Web::HTML::ReplicatedNavigableState activated_navigable_state)
{
    auto navigable = find(navigable_id);
    if (!navigable.has_value())
        return;
    ...
    navigable->did_commit_navigation(move(activated_navigable_state), navigation_id, move(destination_browsing_context));
...
void CanonicalTraversable::did_receive_changing_navigable_continuation_applied(WebContentClient& source_client, u64 source_page_id, Web::HTML::CrossProcessId operation_id, Web::HTML::CrossProcessId navigable_id, Optional<Web::HTML::ReplicatedNavigableState> activated_navigable_state, Optional<Web::HTML::SessionHistoryEntryPersistedState> previous_entry_persisted_state)
{
    if (auto* operation = find_history_operation(operation_id)) {
        ...
        if (activated_navigable_state.has_value())
            did_activate_history_entry(*operation, navigable_id, source_client, source_page_id, pending_job.value()->job.target_entry, pending_job.value()->browsing_context, activated_navigable_state.release_value());
        else
            discard_pending_host_at(navigable_id, *endpoint);
