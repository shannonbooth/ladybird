...
// The child's document went with the page that hosted it. The page holding its container hosts a stand-in for the
// current entry's document until the next document, as the root of an embedded page does.
void CanonicalTraversable::stand_in_for_lost_document(CanonicalNavigable& navigable)
{
    ...
    if (!current_entry)
        return;
    navigable.reporting_page()->async_host_navigable(navigable.id(), current_entry->descriptor());
}
...
// The superseded traversal is canceled, with the navigation it was applying, and the new one takes its place in the
// traversal queue.
...
RefPtr<WebContentPage> CanonicalTraversable::page_hosting(CanonicalNavigable const& navigable) const
{
    ...
    // The traversable's document is hosted by the page displaying the tab.
    if (&navigable == this)
        return display_page();
    return {};
}
...
// The page a changing job runs in: the one hosting the document populated for the job's target entry, which takes
// the container over once the job activates it, or the navigable's document until then.
...
void CanonicalTraversable::finalize_a_same_document_navigation(HistoryOperation& operation, Web::FinalizeSameDocumentNavigationHistoryOperationParameters const& request)
{
    ...
    // AD-HOC: The synchronous navigations that followed this one moved the active entry on, as back-to-back
    //         pushState() calls do. Firefox and Chromium keep every entry, so targetEntry's document state is
    //         compared instead.
    //
