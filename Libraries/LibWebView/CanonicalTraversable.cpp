...
WebContentSessionHistoryUpdateDecision CanonicalTraversable::did_receive_web_content_session_history_update(Vector<Web::HTML::SessionHistoryEntryDescriptor> entries, Vector<i32> used_steps, size_t current_used_step_index, URL::URL const& current_url)
{
    if (m_pending_web_content_session_history_seed.waiting_for_ack)
        return { .status = WebContentSessionHistoryUpdateStatus::IgnoredBeforeSeedAck };
    ...
    if (m_pending_web_content_session_history_seed.step_after_loading_top_level_entry.has_value()) {
        if (current_used_step_index >= used_steps.size() || used_steps[current_used_step_index] != *m_pending_web_content_session_history_seed.step_after_loading_top_level_entry)
            return { .status = WebContentSessionHistoryUpdateStatus::IgnoredPartialBeforeFallbackSeed };
    ...
    if (m_pending_web_content_session_history_seed.ignore_updates_until_seed)
        return { .status = WebContentSessionHistoryUpdateStatus::IgnoredBeforeSeed };

    return {
        .status = WebContentSessionHistoryUpdateStatus::Updated,
...
WebContentSessionHistoryUpdateDecision CanonicalTraversable::did_receive_web_content_session_history_update_for_testing(Vector<Web::HTML::SessionHistoryEntryDescriptor> entries, Vector<i32> used_steps, size_t current_used_step_index, URL::URL const& current_url)
{
    ...
    if (m_pending_web_content_session_history_seed.waiting_for_ack) {
        ...
        if (update.update_result == TraversableSessionHistory::UpdateResult::InvalidSnapshot)
            return { .status = WebContentSessionHistoryUpdateStatus::IgnoredRejectedSeedSnapshotForTesting, .update = move(update) };
        return { .status = WebContentSessionHistoryUpdateStatus::Updated, .update = move(update) };
    }
...
NavigationStartResult CanonicalTraversable::did_start_navigation(URL::URL const& url, Variant<Empty, String, Web::HTML::POSTResource> document_resource, bool is_redirect, Web::Bindings::NavigationHistoryBehavior history_handling, bool is_showing_crash_page)
{
    ...
    if (is_showing_crash_page) {
        if (auto const* current_entry = m_session_history.current_entry(); current_entry && current_entry->url == url) {
            prepare_to_seed_web_content_session_history_from_ui_process();
            return { .status = NavigationStartStatus::StartedFromCrashPage, .did_clear_crash_page = true };
        }
    }
    ...
    if (is_redirect) {
        ...
        m_current_web_content_session_history_matches_mirror = false;
        return { .status = NavigationStartStatus::Redirect, .should_update_navigation_action_state = true, .should_update_webdriver_pending_navigation_url = true, .did_clear_crash_page = is_showing_crash_page };
    }

    if (auto const* current_entry = m_session_history.current_entry(); current_entry && current_entry->url == url) {
        if (m_pending_session_history_navigation.has_value() && m_pending_session_history_navigation->url == url)
            return { .status = NavigationStartStatus::CurrentEntryUnchanged, .did_clear_crash_page = is_showing_crash_page };
        ...
        if (history_handling == Web::Bindings::NavigationHistoryBehavior::Replace) {
            m_session_history.replace_current_entry(url, move(document_resource));
            m_current_web_content_session_history_matches_mirror = false;
            return { .status = NavigationStartStatus::ReplacedCurrentEntry, .should_update_navigation_action_state = true, .did_clear_crash_page = is_showing_crash_page };
        }
        if (history_handling == Web::Bindings::NavigationHistoryBehavior::Push) {
            m_session_history.navigate(url, move(document_resource));
            m_current_web_content_session_history_matches_mirror = false;
            return { .status = NavigationStartStatus::PushedCurrentEntry, .should_update_navigation_action_state = true, .did_clear_crash_page = is_showing_crash_page };
        }
        return { .status = NavigationStartStatus::CurrentEntryUnchanged, .did_clear_crash_page = is_showing_crash_page };
    }
    ...
    if (history_handling == Web::Bindings::NavigationHistoryBehavior::Replace)
        ...
        m_session_history.navigate(url, move(document_resource));
    m_current_web_content_session_history_matches_mirror = false;
    return { .status = NavigationStartStatus::Started, .should_update_navigation_action_state = true, .did_clear_crash_page = is_showing_crash_page };
}
...
WebContentHistoryStepResult CanonicalTraversable::did_traverse_the_history_to_step(i32 step, bool step_was_available, Web::HTML::HistoryStepResult result)
{
    if (!m_pending_web_content_session_history_seed.step_after_loading_top_level_entry.has_value()) {
        ...
        if (!step_was_available) {
            ...
            if (target.has_value())
                return { .status = WebContentHistoryStepStatus::FallbackLoad, .fallback_target = *target };
            m_current_web_content_session_history_matches_mirror = false;
            m_session_history.forget_web_content_state();
            m_pending_session_history_traversal.clear();
            return { .status = WebContentHistoryStepStatus::Unavailable };
        }

        if (result != Web::HTML::HistoryStepResult::Applied) {
            m_pending_session_history_traversal.clear();
            return { .status = WebContentHistoryStepStatus::Canceled, .should_complete_webdriver_pending_navigation = true, .should_update_webdriver_pending_navigation_to_current_url = true, .should_reset_webdriver_pending_navigation_completion = true };
        }
        ...
        m_pending_session_history_traversal.clear();
        return { .status = WebContentHistoryStepStatus::Applied, .current_url = move(current_url), .should_complete_webdriver_pending_navigation = should_complete_webdriver_pending_navigation };
    }
    ...
    if (step_was_available && result == Web::HTML::HistoryStepResult::Applied) {
        ...
        m_pending_session_history_traversal.clear();
        return { .status = WebContentHistoryStepStatus::Restored, .should_complete_webdriver_pending_navigation = true };
    }

    if (m_pending_session_history_navigation.has_value())
        return { .status = WebContentHistoryStepStatus::RestorePendingNavigation, .pending_step_was_available = step_was_available };

    m_pending_web_content_session_history_seed.step_after_loading_top_level_entry.clear();
    m_current_web_content_session_history_matches_mirror = false;
    m_session_history.forget_web_content_state();
    m_pending_session_history_traversal.clear();
    return { .status = WebContentHistoryStepStatus::PendingStepFailed, .pending_step_was_available = step_was_available };
}
...
HistoryStepCancelationCheckResult CanonicalTraversable::did_check_if_traverse_history_step_is_canceled(u64 request_id, i32 step, bool canceled)
{
    ...
    if (canceled) {
        ...
        m_pending_session_history_traversal.clear();
        return { .status = HistoryStepCancelationCheckStatus::Canceled, .on_cancelation_check_complete = move(on_cancelation_check_complete), .outcome = { .status = HistoryTraversalStatus::Canceled }, .should_update_webdriver_pending_navigation_to_current_url = true, .should_reset_webdriver_pending_navigation_completion = true };
    }
    ...
    if (!target.has_value()) {
        ...
        m_session_history.forget_web_content_state();
        m_pending_session_history_traversal.clear();
        return { .status = HistoryStepCancelationCheckStatus::NoTarget, .on_cancelation_check_complete = move(on_cancelation_check_complete), .outcome = { .status = HistoryTraversalStatus::NoEntry } };
    }

    auto on_cancelation_check_complete = move(m_pending_session_history_traversal->on_cancelation_check_complete);
    return {
        .status = HistoryStepCancelationCheckStatus::LoadTarget,
