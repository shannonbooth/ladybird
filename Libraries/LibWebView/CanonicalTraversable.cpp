...
WebContentSessionHistoryReportResult CanonicalTraversable::did_receive_web_content_session_history_update(Web::HTML::WebContentSessionHistoryUpdate update)
{
    ...
    if (m_pending_web_content_session_history_seed.waiting_for_ack)
        return { .dump_reason = "ignored-session-history-update-before-ui-seed-ack"sv };

    if (m_pending_web_content_session_history_seed.ignore_updates_until_seed)
        return { .dump_reason = "ignored-session-history-update-before-ui-seed"sv };

    auto top_level_cross_document_navigation_commit_url = Optional<URL::URL> {};
    if (update.details.has<Web::HTML::TopLevelCrossDocumentNavigationCommitted>())
        top_level_cross_document_navigation_commit_url = update.details.get<Web::HTML::TopLevelCrossDocumentNavigationCommitted>().url;
    ...
    if (top_level_cross_document_navigation_commit_url.has_value()) {
        if (auto const* current_entry = m_session_history.current_entry(); current_entry && current_entry->url == *top_level_cross_document_navigation_commit_url)
            m_pending_session_history_navigation.clear();

        auto navigation_commit_establishes_complete_mirror = pending_navigation_started_from_complete_mirror
            || (m_session_history.size() == 1 && m_session_history.used_step_count() == 1);
        if (navigation_commit_establishes_complete_mirror)
            m_session_history.did_confirm_web_content_matches_mirror();
    }
...
WebContentSessionHistoryUpdateDecision CanonicalTraversable::did_receive_web_content_session_history_update_for_testing(Vector<Web::HTML::SessionHistoryEntryDescriptor> entries, Vector<i32> used_steps, size_t current_used_step_index, URL::URL const& current_url)
{
    ...
    //     allow this testing hook to adopt the explicit snapshot it requested.
    if (m_pending_web_content_session_history_seed.waiting_for_ack) {
...
WebContentSessionHistorySeedAckResult CanonicalTraversable::did_receive_web_content_session_history_seed_ack(u64 seed_id, bool accepted)
{
    if (!m_pending_web_content_session_history_seed.waiting_for_ack)
        return { .ignored = true, .dump_reason = "ignored-webcontent-session-history-seed-ack"sv };
    if (seed_id != m_pending_web_content_session_history_seed.seed_id)
    ...
    if (!m_pending_web_content_session_history_seed.current_top_level_entry_index.has_value()
        || !m_session_history.did_seed_web_content_from_ui_process(*m_pending_web_content_session_history_seed.current_top_level_entry_index)) {
...
Optional<WebContentSessionHistorySeed> CanonicalTraversable::prepare_web_content_session_history_seed(bool allow_current_entry_reconstruction)
{
    ...
    m_pending_web_content_session_history_seed.seed_id = m_next_web_content_session_history_seed_id++;
    m_pending_web_content_session_history_seed.current_top_level_entry_index = *current_top_level_entry_index;

    return WebContentSessionHistorySeed {
        .seed_id = m_pending_web_content_session_history_seed.seed_id,
