...
public:
    ...
    void for_each_page_representing(CanonicalNavigable const&, Function<void(WebContentClient&, u64 page_id)> const&) const;
...
private:
    ...
    void did_activate_history_entry(HistoryOperation&, Web::HTML::CrossProcessId navigable_id, WebContentClient& source_client, u64 source_page_id, Web::HTML::SessionHistoryEntryDescriptor const& target_entry, RefPtr<CanonicalBrowsingContext> destination_browsing_context, Web::HTML::ReplicatedNavigableState activated_navigable_state);
    // Whether step 6 of unload a document and its descendants unloaded the document in the page hosting it, rather
    // than leaving that to the task it queues in the page continuing the invoking algorithm.
