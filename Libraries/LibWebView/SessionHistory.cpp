...
{
    // AD-HOC: The HTML Standard runs "traverse the history by a delta" on the session history traversal queue, so a
    //         queued traversal resolves its delta after earlier queued traversals have already updated the current
    //         session history step. The UI process does not advance its history mirror's current step until WebContent
    //         reports the step applied, so a traversal that compounds onto an in-flight traversal must resolve its
    //         delta from that traversal's target step to match the queue's behavior.
