...
bool TraversableSessionHistory::update_top_level_scroll_position_data(Utf16String const& navigation_api_key, Web::HTML::SessionHistoryEntryScrollPositionData scroll_position_data)
{
    return update_top_level_session_history_entries_by_navigation_api_key(m_entries, m_web_content_known_entries, navigation_api_key, [&](Entry& entry) {
...
bool TraversableSessionHistory::update_nested_scroll_position_data(Web::HTML::CrossProcessId nested_history_id, Utf16String const& navigation_api_key, Web::HTML::SessionHistoryEntryScrollPositionData scroll_position_data)
{
    return update_nested_session_history_entries_by_navigation_api_key(m_entries, m_web_content_known_entries, nested_history_id, navigation_api_key, [&](Entry& entry) {
        entry.scroll_position_data = scroll_position_data;
    });
...
bool TraversableSessionHistory::update_top_level_document_state_navigable_target_name(Utf16String const& navigation_api_key, Utf16String navigable_target_name)
{
    auto update_document_state = [&](auto& document_state) {
...
bool TraversableSessionHistory::update_nested_document_state_navigable_target_name(Web::HTML::CrossProcessId nested_history_id, Utf16String const& navigation_api_key, Utf16String navigable_target_name)
{
    auto update_document_state = [&](auto& document_state) {
        document_state.navigable_target_name = navigable_target_name;
    };
...
bool TraversableSessionHistory::set_top_level_document_state_reload_pending(Utf16String const& navigation_api_key, bool reload_pending)
{
    auto update_document_state = [reload_pending](auto& document_state) {
...
bool TraversableSessionHistory::set_nested_document_state_reload_pending(Web::HTML::CrossProcessId nested_history_id, Utf16String const& navigation_api_key, bool reload_pending)
{
    auto update_document_state = [reload_pending](auto& document_state) {
        document_state.reload_pending = reload_pending;
    };
...
bool TraversableSessionHistory::remove_nested_history(CanonicalNavigable const& parent_navigable, Web::HTML::CrossProcessId child_navigable_id)
{
    ...
    if (!m_used_steps.contains_slow(current_step)) {
        ...
        // Return the greatest integer in steps that is less than or equal to step.
...
bool TraversableSessionHistory::finalize_same_document_navigation(CanonicalNavigable const& navigable, Entry target_entry, Optional<Utf16String> entry_to_replace_navigation_api_key)
{
    ...
    auto update_target_entries = [&](auto& entries, auto const& entry) {
    ...
    auto did_update = navigable.is_top_level_traversable()
        ? update_target_entries(m_entries, canonical_target_entry)
        : update_nested_session_history_entries_for_navigable(m_entries, navigable.id(), [&](auto& entries) {
              return update_target_entries(entries, canonical_target_entry);
          });
    if (!did_update)
        return false;

    if (navigable.is_top_level_traversable() && !m_web_content_known_entries.is_empty()) {
        update_target_entries(m_web_content_known_entries, target_entry);
        ...
        update_nested_session_history_entries_for_navigable(m_web_content_known_entries, navigable.id(), [&](auto& entries) {
            return update_target_entries(entries, target_entry);
        });
    }
