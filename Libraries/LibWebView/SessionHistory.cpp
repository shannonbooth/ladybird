...
static bool reported_entry_matches_target_except_updated_field(TraversableSessionHistory::Entry const& stored_entry, TraversableSessionHistory::Entry const& reported_entry, Web::HTML::SessionHistoryEntryUpdateKind update_kind)
{
    auto expected_entry = reported_entry;
    switch (update_kind) {
...
static bool apply_targeted_current_entry_update(Vector<TraversableSessionHistory::Entry>& entries, TraversableSessionHistory::Entry const& updated_entry, Web::HTML::SessionHistoryEntryUpdateKind update_kind, bool& found)
{
    ...
    for (auto& entry : entries) {
        ...
        if (should_receive_update) {
            found = true;
            if (entry.step == updated_entry.step
                && entry.document_state.id == updated_entry.document_state.id
                && !reported_entry_matches_target_except_updated_field(entry, updated_entry, update_kind)) {
                return false;
            }
...
static TraversableSessionHistory::Entry entry_from_same_document_navigation(Web::HTML::SameDocumentNavigationCommitted navigation)
{
    return {
        .step = navigation.replaced_step.value_or(navigation.current_step),
...
static TraversableSessionHistory::Entry entry_from_nested_same_document_navigation(Web::HTML::NestedSameDocumentNavigationCommitted navigation)
{
    return {
        .step = navigation.replaced_step.value_or(navigation.current_step),
        .url = move(navigation.url),
...
static TraversableSessionHistory::Entry entry_from_top_level_cross_document_navigation(Web::HTML::TopLevelCrossDocumentNavigationCommitted const& navigation)
{
    return {
        .step = navigation.current_step,
        .url = navigation.url,
        .document_state = navigation.document_state,
        .classic_history_api_state = navigation.classic_history_api_state,
        .navigation_api_state = navigation.navigation_api_state,
        .navigation_api_key = navigation.navigation_api_key,
        .navigation_api_id = navigation.navigation_api_id,
        .scroll_restoration_mode = navigation.scroll_restoration_mode,
        .scroll_position_data = navigation.scroll_position_data,
    };
}

static TraversableSessionHistory::Entry entry_from_nested_cross_document_navigation(Web::HTML::NestedCrossDocumentNavigationCommitted const& navigation)
{
    return {
        .step = navigation.current_step,
        .url = navigation.url,
...
static bool append_child_navigable_to_document_state(Vector<TraversableSessionHistory::Entry>& entries, Web::HTML::CrossProcessId parent_document_state_id, Web::HTML::CrossProcessId navigable_id, TraversableSessionHistory::Entry const& initial_entry, bool& found_parent)
{
    for (auto& entry : entries) {
        ...
        for (auto& nested_history : entry.document_state.nested_histories) {
            if (!append_child_navigable_to_document_state(nested_history.entries, parent_document_state_id, navigable_id, initial_entry, found_parent))
                return false;
        }
    }
...
static bool remove_child_navigable_from_document_state(Vector<TraversableSessionHistory::Entry>& entries, Web::HTML::CrossProcessId parent_document_state_id, Web::HTML::CrossProcessId navigable_id, bool& found_parent)
{
    for (auto& entry : entries) {
        if (entry.document_state.id == parent_document_state_id) {
            found_parent = true;
            auto old_size = entry.document_state.nested_histories.size();
            entry.document_state.nested_histories.remove_all_matching([&](auto const& nested_history) {
                return nested_history.id == navigable_id;
            });
            if (entry.document_state.nested_histories.size() == old_size)
                return false;
        }

        for (auto& nested_history : entry.document_state.nested_histories) {
            if (!remove_child_navigable_from_document_state(nested_history.entries, parent_document_state_id, navigable_id, found_parent))
                return false;
        }
    }
...
static Vector<TraversableSessionHistory::Entry>* nested_history_entries_for(Vector<TraversableSessionHistory::Entry>& entries, Web::HTML::CrossProcessId parent_document_state_id, Web::HTML::CrossProcessId navigable_id)
{
    for (auto& entry : entries) {
        if (entry.document_state.id == parent_document_state_id) {
            for (auto& nested_history : entry.document_state.nested_histories) {
        ...
        for (auto& nested_history : entry.document_state.nested_histories) {
            if (auto* nested_entries = nested_history_entries_for(nested_history.entries, parent_document_state_id, navigable_id))
...
static bool apply_same_document_navigation_commit(
    Vector<TraversableSessionHistory::Entry>& entries,
    Vector<i32>& used_steps,
    Optional<size_t>& current_used_step_index,
    TraversableSessionHistory::Entry committed_entry,
    i32 current_step,
    Optional<i32> replaced_step,
    Optional<Web::HTML::CrossProcessId> parent_document_state_id = {},
    Optional<Web::HTML::CrossProcessId> navigable_id = {})
{
    if (committed_entry.step < 0 || !current_used_step_index.has_value())
        return false;
    ...
    auto previous_current_step = used_steps[*current_used_step_index];

    Vector<TraversableSessionHistory::Entry>* target_entries = nullptr;
    if (parent_document_state_id.has_value() || navigable_id.has_value()) {
        if (!parent_document_state_id.has_value() || !navigable_id.has_value())
            return false;
        target_entries = nested_history_entries_for(entries, *parent_document_state_id, *navigable_id);
    } else {
    ...
    auto committed_step = committed_entry.step;
    if (replaced_step.has_value()) {
        if (*replaced_step != committed_step)
            return false;

        auto entry_to_replace = target_entries->find_if([&](auto const& entry) {
            return entry.step == *replaced_step;
        });
        if (entry_to_replace == target_entries->end())
            return false;

        *entry_to_replace = move(committed_entry);
    } else {
        if (committed_step != current_step)
            return false;

        if (committed_step <= previous_current_step)
            return false;

        clear_forward_session_history_entries(entries, previous_current_step);

        if (parent_document_state_id.has_value())
            target_entries = nested_history_entries_for(entries, *parent_document_state_id, *navigable_id);
        else
            target_entries = &entries;
        ...
        target_entries->append(move(committed_entry));
    }

    recompute_used_steps(entries, used_steps, current_used_step_index, current_step);
    return current_used_step_index.has_value() && entries_are_valid(entries) && entries_and_used_steps_are_consistent(entries, used_steps);
}

static bool apply_child_navigable_created(
    Vector<TraversableSessionHistory::Entry>& entries,
    Vector<i32>& used_steps,
    Optional<size_t>& current_used_step_index,
    Web::HTML::ChildNavigableCreated const& created)
{
    if (created.current_step < 0 || created.initial_entry.step < 0)
        return false;

    bool found_parent = false;
    if (!append_child_navigable_to_document_state(entries, created.parent_document_state_id, created.navigable_id, created.initial_entry, found_parent) || !found_parent)
        return false;

    recompute_used_steps(entries, used_steps, current_used_step_index, created.current_step);
    return current_used_step_index.has_value() && entries_are_valid(entries) && entries_and_used_steps_are_consistent(entries, used_steps);
}

static bool apply_child_navigable_destroyed(
    Vector<TraversableSessionHistory::Entry>& entries,
    Vector<i32>& used_steps,
    Optional<size_t>& current_used_step_index,
    Web::HTML::ChildNavigableDestroyed const& destroyed)
{
    if (destroyed.current_step < 0)
        return false;

    bool found_parent = false;
    if (!remove_child_navigable_from_document_state(entries, destroyed.parent_document_state_id, destroyed.navigable_id, found_parent) || !found_parent)
        return false;

    recompute_used_steps(entries, used_steps, current_used_step_index, destroyed.current_step);
    return current_used_step_index.has_value() && entries_are_valid(entries) && entries_and_used_steps_are_consistent(entries, used_steps);
}

static bool apply_cross_document_navigation_commit(
    Vector<TraversableSessionHistory::Entry>& entries,
    Vector<i32>& used_steps,
    Optional<size_t>& current_used_step_index,
    TraversableSessionHistory::Entry committed_entry,
    ...
    Optional<Web::HTML::CrossProcessId> navigable_id = {})
{
    if (current_step < 0 || committed_entry.step < 0 || !current_used_step_index.has_value())
        return false;
    ...
    auto previous_current_step = used_steps[*current_used_step_index];
    if (committed_entry.step != current_step)
        return false;
    ...
    if (current_step > previous_current_step) {
        ...
        auto entry_to_replace = target_entries->find_if([&](auto const& entry) {
            return entry.step == current_step;
        });
        if (entry_to_replace == target_entries->end())
            return false;
...
bool TraversableSessionHistory::update_current_entry_from_web_content(Web::HTML::SessionHistoryEntryUpdateKind update_kind, Entry updated_entry)
{
    ...
    if (!apply_targeted_current_entry_update(entries, updated_entry, update_kind, found_entry) || !found_entry)
        return false;

    Optional<Vector<Entry>> updated_web_content_known_entries;
    if (!m_web_content_known_entries.is_empty()) {
        auto web_content_known_entries = m_web_content_known_entries;
        auto found_known_entry = false;
        if (apply_targeted_current_entry_update(web_content_known_entries, updated_entry, update_kind, found_known_entry) && found_known_entry) {
            updated_web_content_known_entries = move(web_content_known_entries);
        } else {
            forget_web_content_state();
        }
    }
    ...
    if (updated_web_content_known_entries.has_value())
        m_web_content_known_entries = updated_web_content_known_entries.release_value();
    return true;
}

bool TraversableSessionHistory::child_navigable_was_created_from_web_content(Web::HTML::ChildNavigableCreated created)
{
    if (!m_web_content_current_step.has_value() || !m_web_content_uses_ui_step_coordinates)
        return false;
    ...
    auto entries = m_entries;
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    if (!apply_child_navigable_created(entries, used_steps, current_used_step_index, created))
        return false;

    auto web_content_known_entries = m_web_content_known_entries;
    auto web_content_known_used_steps = m_web_content_known_used_steps;
    auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
    if (!apply_child_navigable_created(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, created))
        return false;
    ...
    m_web_content_known_used_steps = move(web_content_known_used_steps);
    m_web_content_current_step = m_used_steps[*m_current_used_step_index];
    return true;
}

bool TraversableSessionHistory::child_navigable_was_destroyed_from_web_content(Web::HTML::ChildNavigableDestroyed destroyed)
{
    if (!m_web_content_current_step.has_value() || !m_web_content_uses_ui_step_coordinates)
        return false;
    ...
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    if (!apply_child_navigable_destroyed(entries, used_steps, current_used_step_index, destroyed))
        return false;

    auto web_content_known_entries = m_web_content_known_entries;
    auto web_content_known_used_steps = m_web_content_known_used_steps;
    auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
    if (!apply_child_navigable_destroyed(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, destroyed))
        return false;
...
bool TraversableSessionHistory::commit_same_document_navigation_from_web_content(Web::HTML::SameDocumentNavigationCommitted navigation)
{
    auto replaced_step = navigation.replaced_step;
    auto current_step = navigation.current_step;

    if (!m_current_used_step_index.has_value() || *m_current_used_step_index >= m_used_steps.size())
        return false;

    if (m_web_content_current_step.has_value() && *m_web_content_current_step != m_used_steps[*m_current_used_step_index])
        return false;

    auto entries = m_entries;
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    auto committed_entry = entry_from_same_document_navigation(navigation);
    auto known_committed_entry = entry_from_same_document_navigation(move(navigation));
    if (!apply_same_document_navigation_commit(entries, used_steps, current_used_step_index, move(committed_entry), current_step, replaced_step))
        return false;
    ...
    if (m_web_content_current_step.has_value()) {
        auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
        if (!apply_same_document_navigation_commit(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, move(known_committed_entry), current_step, replaced_step))
            return false;
    } else {
        if (*m_current_used_step_index != m_used_steps.size() - 1)
            return false;
        web_content_known_entries = entries;
        web_content_known_used_steps = used_steps;
    }
...
bool TraversableSessionHistory::commit_nested_same_document_navigation_from_web_content(Web::HTML::NestedSameDocumentNavigationCommitted navigation)
{
    auto replaced_step = navigation.replaced_step;
    auto current_step = navigation.current_step;
    ...
    auto parent_document_state_id = navigation.parent_document_state_id;
    auto navigable_id = navigation.navigable_id;

    auto entries = m_entries;
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    if (!apply_same_document_navigation_commit(entries, used_steps, current_used_step_index, entry_from_nested_same_document_navigation(navigation), current_step, replaced_step, parent_document_state_id, navigable_id))
        return false;

    auto web_content_known_entries = m_web_content_known_entries;
    auto web_content_known_used_steps = m_web_content_known_used_steps;
    auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
    if (!apply_same_document_navigation_commit(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, entry_from_nested_same_document_navigation(move(navigation)), current_step, replaced_step, parent_document_state_id, navigable_id))
        return false;
...
bool TraversableSessionHistory::commit_top_level_cross_document_navigation_from_web_content(Web::HTML::TopLevelCrossDocumentNavigationCommitted navigation)
{
    auto current_step = navigation.current_step;
    ...
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    if (!apply_cross_document_navigation_commit(entries, used_steps, current_used_step_index, entry_from_top_level_cross_document_navigation(navigation), current_step))
        return false;

    auto web_content_known_entries = m_web_content_known_entries;
    auto web_content_known_used_steps = m_web_content_known_used_steps;
    auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
    if (!apply_cross_document_navigation_commit(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, entry_from_top_level_cross_document_navigation(navigation), current_step))
        return false;
...
bool TraversableSessionHistory::commit_nested_cross_document_navigation_from_web_content(Web::HTML::NestedCrossDocumentNavigationCommitted navigation)
{
    auto current_step = navigation.current_step;
    ...
    auto used_steps = m_used_steps;
    auto current_used_step_index = m_current_used_step_index;
    if (!apply_cross_document_navigation_commit(entries, used_steps, current_used_step_index, entry_from_nested_cross_document_navigation(navigation), current_step, parent_document_state_id, navigable_id))
        return false;

    auto web_content_known_entries = m_web_content_known_entries;
    auto web_content_known_used_steps = m_web_content_known_used_steps;
    auto web_content_current_used_step_index = web_content_known_used_steps.find_first_index(*m_web_content_current_step);
    if (!apply_cross_document_navigation_commit(web_content_known_entries, web_content_known_used_steps, web_content_current_used_step_index, entry_from_nested_cross_document_navigation(navigation), current_step, parent_document_state_id, navigable_id))
        return false;
