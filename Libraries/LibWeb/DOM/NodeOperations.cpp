...
WebIDL::ExceptionOr<GC::Ref<Node>> convert_nodes_to_single_node(Vector<Variant<GC::Handle<Node>, String>> const& nodes, DOM::Document& document)
{
    ...
    auto potentially_convert_string_to_text_node = [&document](Variant<GC::Handle<Node>, String> const& node) -> GC::Ref<Node> {
        if (node.has<GC::Handle<Node>>())
            return *node.get<GC::Handle<Node>>();
