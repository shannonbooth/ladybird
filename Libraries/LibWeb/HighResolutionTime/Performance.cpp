...
#undef __ENUMERATE_NAVIGATION_TIMING_ENTRY_NAME
        ...
        auto maybe_entry = performance_entry_buffer.last_matching([&mark_string](GC::Handle<PerformanceTimeline::PerformanceEntry> const& entry) {
...
WebIDL::ExceptionOr<Vector<GC::Handle<PerformanceTimeline::PerformanceEntry>>> Performance::get_entries() const
{
    auto& vm = this->vm();
...
WebIDL::ExceptionOr<Vector<GC::Handle<PerformanceTimeline::PerformanceEntry>>> Performance::get_entries_by_type(String const& type) const
{
    auto& vm = this->vm();
...
WebIDL::ExceptionOr<Vector<GC::Handle<PerformanceTimeline::PerformanceEntry>>> Performance::get_entries_by_name(String const& name, Optional<String> type) const
{
    auto& vm = this->vm();
