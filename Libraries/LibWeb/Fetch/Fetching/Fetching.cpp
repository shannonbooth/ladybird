        ...
        if (cors_check(request, response) && Infrastructure::is_ok_status(response->status())) {
            ...
            if (maybe_max_age && maybe_max_age->size() == 1) {
                if (auto maybe_number = maybe_max_age->first().to_number<i32>(); maybe_number.has_value()) {
                    auto value = maybe_number.value();
                    if (value < 0)
                        max_age = 0;
                    else if (value <= Infrastructure::PreflightCache::MAX_AGE_LIMIT)
                        max_age = static_cast<u16>(value);
                    else
                        max_age = Infrastructure::PreflightCache::MAX_AGE_LIMIT;
                }
            }
