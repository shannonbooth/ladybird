...
static void populate_request_from_client(Infrastructure::Request& request)
{
    ...
    if (request.window() == Infrastructure::Request::Window::Client) {
    ...
    if (request.origin() == Infrastructure::Request::Origin::Client)
        request.set_origin(request.client()->origin());

    // 3. If request’s policy container is "client":
    if (request.policy_container() == Infrastructure::Request::PolicyContainer::Client) {
        ...
        if (request.client()) {
            request.set_policy_container(request.client()->policy_container()->clone(request.heap()));
        }
        // 2. Otherwise, set request’s policy container to a new policy container.
        else {
            request.set_policy_container(request.heap().allocate<HTML::PolicyContainer>(request.heap()));
        }
    }
}
...
WebIDL::ExceptionOr<GC::Ref<Infrastructure::FetchController>> fetch(JS::Realm& realm, Infrastructure::Request& request, Infrastructure::FetchAlgorithms const& algorithms, UseParallelQueue use_parallel_queue)
{
    ...
    // FIXME: 6. If useParallelQueue is true, then set taskDestination to the result of starting a new parallel queue.
    ...
    // 13. If request’s priority is null, then use request’s initiator, destination, and render-blocking appropriately
