...
public:
    static constexpr u16 MAX_AGE_LIMIT = 7200; // 2 hours.

    static PreflightCache& the();

    void cache_method(Request const&, ByteString const& method, u16 max_age);
    void cache_header_name(Request const&, ByteString const& header_name, u16 max_age);
...
private:
    ...
    struct Entry {
        ...
        void update_max_age(u16 max_age);
    };

    void create_a_new_cache_entry(Request const&, u16 max_age, Optional<ByteString> method, Optional<ByteString> header_name);
