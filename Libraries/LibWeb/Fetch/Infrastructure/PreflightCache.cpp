...
{
    static PreflightCache s_cache;
    return s_cache;
}

// https://fetch.spec.whatwg.org/#concept-cache-create-entry
void PreflightCache::create_a_new_cache_entry(Request const& request, u16 max_age, Optional<ByteString> method, Optional<ByteString> header_name)
{
...
void PreflightCache::cache_method(Request const& request, ByteString const& method, u16 max_age)
{
    evict_expired_entries();
...
// https://fetch.spec.whatwg.org/#concept-cache-match-header-name
bool PreflightCache::is_a_header_name_cache_entry_match(Request const& request, ByteString const& header_name)
{
    ...
    // agent’s CORS-preflight cache for which there is a cache entry match with request and its header name is a
    // byte-case-insensitive match for headerName or `*`.
...
{
    for (auto& entry : m_entries) {
        if (!entry.header_name.has_value() || (!entry.header_name->equals_ignoring_ascii_case(header_name) && entry.header_name.value() != "*"sv))
            continue;
        if (is_cache_entry_match(entry, request))
            return &entry;
    }
...
void PreflightCache::cache_header_name(Request const& request, ByteString const& header_name, u16 max_age)
{
    evict_expired_entries();
...
{
    return MonotonicTime::now() - stored_at >= AK::Duration::from_seconds(max_age);
}

void PreflightCache::Entry::update_max_age(u16 new_max_age)
{
    max_age = new_max_age;
    stored_at = MonotonicTime::now();
}
