...
private:
    ...
    struct UIHistoryOperationState {
        ...
        // Navigables a changing job claimed for "traversal" whose continuation has not applied yet; the sweep at
        // completion releases claims a canceled or stale operation left behind.
        HashTable<CrossProcessId> claimed_navigables_awaiting_continuation;
        // A continuation owns live document state and therefore remains in the process that produced it. Keep it
        // under the UI operation that requested the job so nested operations for one navigable cannot collide.
