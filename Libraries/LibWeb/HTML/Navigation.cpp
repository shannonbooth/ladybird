...
WebIDL::ExceptionOr<Bindings::NavigationResult> Navigation::perform_a_navigation_api_traversal(Utf16String key, Bindings::NavigationOptions const& options)
{
    ...
    traversable->append_session_history_traversal_steps(GC::create_function(heap(), [key, api_method_tracker, navigable, source_snapshot_params, traversable, this](NonnullRefPtr<Core::Promise<Empty>> signal) {
        auto reject_finished_promise_with_invalid_state_error = GC::create_function(heap(), [this, api_method_tracker] {
        ...
        // 3. If targetSHE is navigable's active session history entry, reject this stale queued request.
        if (auto active_entry = navigable->active_session_history_entry(); active_entry && active_entry->navigation_api_key() == key) {
            reject_finished_promise_with_invalid_state_error->function()();
                signal->resolve({});
                return;
            }

        // The UI process performs target lookup and apply the traverse history step as one queued operation. Keeping
        ...
        traversable->page().client().page_did_request_history_traversal(
            ...
            GC::create_function(heap(), [this, api_method_tracker, reject_finished_promise_with_invalid_state_error](HistoryStepResult result) {
                ...
                // NOTE: When result is "canceled-by-beforeunload" or "initiator-disallowed", the navigate event was
                // never fired, so aborting the ongoing navigation would incorrectly fire navigateerror.
                auto& realm = relevant_realm(*this);
                ...
                } else if (result == HistoryStepResult::InitiatorDisallowed) {
                    queue_global_task(Task::Source::NavigationAndTraversal, global, GC::create_function(heap(), [this, api_method_tracker, &realm] {
                        TemporaryExecutionContext execution_context { realm };
