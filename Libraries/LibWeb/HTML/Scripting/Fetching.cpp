...
WebIDL::ExceptionOr<URL::URL> resolve_module_specifier(Optional<Script&> referring_script, ByteString const& specifier)
{
    ...
    // 6. Let serializedBaseURLString be baseURL, serialized.
    auto serialized_base_url_string = base_url->serialize();
    ...
    for (auto const& entry : import_map.scopes()) {
        ...
        // 1. If scopePrefix is serializedBaseURLString, or if scopePrefix ends with U+002F (/) and scopePrefix is a code unit prefix of serializedBaseURLString, then:
        if (scope_prefix == serialized_base_url_string || (scope_prefix.ends_with("/"sv) && Infra::is_code_unit_prefix(scope_prefix, serialized_base_url_string))) {
    ...
    // Spec-Note: At this point, if result is null, specifier wasn't remapped to anything by importMap, but it might have been able to be turned into a URL.
    if (!result.has_value())
        result = as_url;

    // 13. If asURL is not null, then:
    if (as_url.has_value()) {
        // 1. Add module to resolved module set given realm, serializedBaseURLString, normalizedSpecifier, and asURL.
        add_module_to_resolved_module_set(*realm, MUST(String::from_byte_string(serialized_base_url_string)), MUST(String::from_byte_string(normalized_specifier)), as_url);
...
// FIXME: What about https://html.spec.whatwg.org/multipage/webappapis.html#fetch-a-modulepreload-module-script-graph
