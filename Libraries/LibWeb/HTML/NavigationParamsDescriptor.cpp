...
static NavigationResponseDescriptor create_navigation_response_descriptor(Fetch::Infrastructure::Response const& response)
{
    ...
    Optional<NavigationResponseBodyHandle> body_handle;
    if (auto const& request = response.request_server_request(); request.has_value()) {
        body_handle = NavigationResponseBodyHandle {
            .request_server_client_id = request->client_id,
            .request_server_request_id = request->request_id,
        };
    }

    Optional<ByteBuffer> body_bytes;
    if (!body_handle.has_value() && response.body())
        body_bytes = copy_body_source(*response.body());
    ...
    return {
        ...
        .body_handle = move(body_handle),
        .body_bytes = move(body_bytes),
    };
}
...
void create_navigation_params_descriptor(JS::Realm& realm, NavigationParamsVariant params, GC::Ref<NavigationParamsDescriptorCompletion> completion_steps)
{
    ...
    if (!body || descriptor.response.body_handle.has_value() || descriptor.response.body_bytes.has_value()) {
    ...
    body->fully_read(
        ...
        GC::create_function(realm.heap(), [descriptor = move(descriptor), completion_steps](ByteBuffer bytes) mutable {
            descriptor.response.body_bytes = move(bytes);
            completion_steps->function()(move(descriptor));
        }),
...
static GC::Ref<Fetch::Infrastructure::Response> create_navigation_response_from_descriptor(JS::Realm& realm, NavigationResponseDescriptor descriptor)
{
    VERIFY(!(descriptor.body_handle.has_value() && descriptor.body_bytes.has_value()));
    ...
    if (descriptor.body_bytes.has_value()) {
        response->set_body(Fetch::Infrastructure::byte_sequence_as_body(realm, descriptor.body_bytes->bytes()));
    } else if (descriptor.body_handle.has_value()) {
        response->set_body(adopt_navigation_response_body(realm, *descriptor.body_handle, response));
    }
    return response;
}

NavigationParamsVariant create_navigation_params_from_descriptor(JS::Realm& realm, LocalNavigable& navigable, NavigationParamsVariantDescriptor descriptor)
{
    ...
    auto response = create_navigation_response_from_descriptor(realm, move(params.response));
...
ErrorOr<void> encode(Encoder& encoder, Web::HTML::NavigationResponseDescriptor const& response)
{
    ...
    TRY(encoder.encode(response.body_handle));
    TRY(encoder.encode(response.body_bytes));
    return {};
}
...
ErrorOr<Web::HTML::NavigationResponseDescriptor> decode(Decoder& decoder)
{
    return Web::HTML::NavigationResponseDescriptor {
        ...
        .body_handle = TRY(decoder.decode<Optional<Web::HTML::NavigationResponseBodyHandle>>()),
        .body_bytes = TRY(decoder.decode<Optional<ByteBuffer>>()),
    };
}
