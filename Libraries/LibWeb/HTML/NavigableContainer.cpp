...
void NavigableContainer::create_new_child_navigable()
{
    ...
    // NB: group's only use is step 9 of creating a new browsing context and document, obtaining the similar-origin
    //     window agent. This process does not model agents, and the UI process places documents in agents from its
    //     canonical browsing context group, so group is not resolved here.
