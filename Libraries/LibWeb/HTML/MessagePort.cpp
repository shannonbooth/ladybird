...
WebIDL::ExceptionOr<void> MessagePort::transfer_steps(HTML::TransferDataEncoder& data_holder)
{
    ...
    // Quiesce the transport before draining it so already-received messages and peer shutdown state
    // move with the transferred MessagePort instead of racing with the snapshot.
    drain_transport();
    ...
    if (has_remote_port_handle) {
        ...
        data_holder.encode(move(remote_port_handle));
    }
...
WebIDL::ExceptionOr<void> MessagePort::message_port_post_message_steps(GC::Ptr<MessagePort> target_port, JS::Value message, StructuredSerializeOptions const& options)
{
    ...
    // 6. If doomed is true, then return.
    if (doomed) {
        return {};
    }

    // IMPLEMENTATION DEFINED: Queue outbound messages while a port exists but is not yet entangled.
    if (!m_transport) {
        if (!is_detached())
