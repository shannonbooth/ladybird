...
{
    // A navigation given a response was made by this process for a navigable it hosts; none is routed elsewhere.
    VERIFY(!navigation.response);

    // NB: The form data entry list is read only by step 21 of navigate, for a target whose active document is same
    //     origin-domain with the source document, and such documents share a process. The source element is an
    //     element of the source document, so the navigate event the target fires for this request sees null.
    return {
        .url = navigation.url,
