...
{
    auto had_navigation_parked_for_population = m_pending_navigations.find_first_index_if([](auto const& pending) {
...
void LocalNavigable::begin_navigation(PreparedNavigation navigation)
{
    ...
    if (history_handling == Bindings::NavigationHistoryBehavior::Auto) {
        // 1. If url equals navigable's active document's URL, and either sourceDocument is null or
        //    initiatorOriginSnapshot is same origin with navigable's active document's origin, then set
        //    historyHandling to "replace".
        // AD-HOC: The sourceDocument-is-null alternative is not yet in the HTML Standard. It matches
        //         Firefox, Chromium, and WebKit when browser UI navigates to the current URL.
        if (url == active_document.url() && (!source_document || initiator_origin_snapshot.is_same_origin(active_document.origin())))
