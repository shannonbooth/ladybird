    ...
    LocalNavigable& parent_navigable, LocalNavigable& child_navigable, SessionHistoryEntry& history_entry)
{
    ...
    SessionHistoryNestedHistoryDescriptor nested_history_descriptor {
        .id = child_navigable.id(),
        .entries { traversable->create_session_history_entry_descriptor_for_ui_process(history_entry) },
    };
    ...
    if (!chosen) {
        ...
        // --> If the user agent has been configured such that in this instance it will create a new top-level traversable
            ...
            auto create_new_traversable_closure = [this, no_opener, target_name, activate_tab, window_features](GC::Ptr<BrowsingContext> opener) -> GC::Ref<LocalNavigable> {
                ...
                // Step 9 appends the initial history entry to the traversable's session history entries. Report that
                // structural operation so the UI process can initialize the canonical traversable before processing
                // the URL and history update steps that follow window creation.
                ...
                page->client().page_did_create_top_level_traversable(
                    traversable->id(),
                    traversable->create_session_history_entry_descriptor_for_ui_process(*initial_history_entry));
                return traversable;
            };
...
void LocalNavigable::navigate_to_a_fragment(URL::URL const& url, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement, GC::Ptr<DOM::Element> source_element, Optional<StorageSerializationRecord> navigation_api_state, Utf16String navigation_id)
{
    ...
    traversable->request_to_finalize_same_document_navigation(*this, history_entry, active_entry->navigation_api_key(), entry_to_replace);
...
void perform_url_and_history_update_steps(DOM::Document& document, URL::URL new_url, Optional<StorageSerializationRecord> serialized_data, HistoryHandlingBehavior history_handling)
{
    ...
    traversable->request_to_finalize_same_document_navigation(*navigable, new_entry, active_entry->navigation_api_key(), entry_to_replace);
