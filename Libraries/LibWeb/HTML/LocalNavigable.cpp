...
void LocalNavigable::prepare_child_navigable_history_reconstruction(SessionHistoryDocumentStateDescriptor const& document_state_descriptor)
{
    ...
    if (active_entry && active_document
        ...
        if (child_navigables.size() == child_navigable_ids.size()) {
            ...
            // NB: A document this process created has its child navigables created here as well.
...
// The focus algorithms ask this of a top-level traversable. Where that traversable is hosted by another process, the
// navigable rooting this process's part of the tree stands in for it: the page's system focus and the focused areas of
// the documents hosted here are what this process knows of focus.
