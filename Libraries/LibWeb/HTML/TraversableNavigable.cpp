...
GC::Ptr<WindowProxy> TraversableNavigable::active_window_proxy()
{
...
{
    return m_navigable->is_top_level_traversable();
}
...
void TraversableNavigable::visit_edges(Cell::Visitor& visitor)
{
    ...
    m_emulated_position_data.visit(
        [](Empty const&) {},
        [&](GC::Ref<Geolocation::GeolocationCoordinates> const& coordinates) {
            visitor.visit(coordinates);
        },
        [](Geolocation::GeolocationPositionError::ErrorCode const&) {});
}
...
GC::Ref<LocalTraversableNavigable> LocalTraversableNavigable::create_a_fresh_top_level_traversable(GC::Ref<Page> page, URL::URL const& initial_navigation_url, Variant<Empty, String, POSTResource> initial_navigation_post_resource)
{
    ...
    traversable->traversable_navigable()->set_emulated_position_data(emulated_position_coordinates);
...
void LocalTraversableNavigable::check_if_unloading_is_canceled(Vector<GC::Root<Navigable>> navigables_that_need_before_unload, GC::Ref<GC::Function<void(CheckIfUnloadingIsCanceledResult)>> callback)
{
