...
static void continue_navigation_at_population(GC::Ref<LocalNavigable> navigable, NavigationPopulationRequest request, NavigationPopulationResult result)
{
    ...
    auto navigation_params = create_navigation_params_from_descriptor(realm, *navigable, move(result.navigation_params));
...
{
    auto navigable = local_navigable_with_id(request.navigable_id);
    if (!navigable || navigable->traversable_navigable().ptr() != this)
        return;
    Web::HTML::continue_navigation_at_population(*navigable, move(request), move(result));
}
