...
public:
    ...
    {
        m_traversable->start_apply_history_step_operation(m_generation, m_history_initiation_id, m_navigation_api_abort_behavior);
        m_timeout->start();
    }
...
bool LocalTraversableNavigable::run_changing_navigable_history_step_job_impl(ChangingNavigableHistoryStepJob job, GC::Ptr<SourceSnapshotParams> source_snapshot_params, GC::Ptr<DOM::Document> pending_document, GC::Ref<OnLocalChangingNavigableHistoryStepJobComplete> on_complete)
{
    ...
    // NB: Do not replace a newer cross-document navigation's ID with "traversal" when applying a queued
...
void ApplyHistoryStepState::complete()
{
    ...
    m_traversable->complete_apply_history_step_operation(m_generation);
...
void LocalTraversableNavigable::finalize_same_document_navigation(GC::Ref<LocalNavigable> target_navigable, NonnullRefPtr<SessionHistoryEntry> target_entry, RefPtr<SessionHistoryEntry> entry_to_replace, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement)
{
    ...
    if (m_apply_history_step_state || m_paused_apply_history_step_state || !target_navigable->has_session_history_entry_and_ready_for_navigation()) {
        ...
        request_synchronous_navigation_history_operation(target_navigable, move(parameters), { .pre_steps = pre_steps });
        return;
    }
