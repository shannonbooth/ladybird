...
using MessageEventSource = Variant<GC::Handle<WindowProxy>, GC::Handle<MessagePort>>;
...
struct MessageEventInit : public DOM::EventInit {
    ...
    Vector<GC::Handle<MessagePort>> ports;
};
...
public:
    ...
    Variant<GC::Handle<WindowProxy>, GC::Handle<MessagePort>, Empty> source() const;

    void init_message_event(String const& type, bool bubbles, bool cancelable, JS::Value data, String const& origin, String const& last_event_id, Optional<MessageEventSource> source, Vector<GC::Handle<MessagePort>> const& ports);
