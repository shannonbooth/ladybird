...
using IDBDatabaseMapping = HashMap<StorageAPI::StorageKey, HashMap<String, GC::Handle<Database>>>;
...
Optional<GC::Handle<Database>> Database::for_key_and_name(StorageAPI::StorageKey& key, String& name)
{
    return m_databases.ensure(key, [] {
                          return HashMap<String, GC::Handle<Database>>();
                      })
        .get(name);
}

ErrorOr<GC::Handle<Database>> Database::create_for_key_and_name(JS::Realm& realm, StorageAPI::StorageKey& key, String& name)
{
