...
void WebGL2RenderingContextOverloads::tex_image2d(WebIDL::UnsignedLong target, WebIDL::Long level, WebIDL::Long internalformat, WebIDL::Long width, WebIDL::Long height, WebIDL::Long border, WebIDL::UnsignedLong format, WebIDL::UnsignedLong type, Optional<GC::Root<WebIDL::ArrayBufferView>> pixels)
{
    m_context->make_current();

    ReadonlyBytes pixels_span;
    if (pixels.has_value() && pixels.value()) {
        pixels_span = SET_ERROR_VALUE_IF_ERROR(get_offset_span<u8 const>(*pixels.value(), /* src_offset= */ 0), GL_INVALID_OPERATION);
    }
...
void WebGL2RenderingContextOverloads::tex_sub_image2d(WebIDL::UnsignedLong target, WebIDL::Long level, WebIDL::Long xoffset, WebIDL::Long yoffset, WebIDL::Long width, WebIDL::Long height, WebIDL::UnsignedLong format, WebIDL::UnsignedLong type, Optional<GC::Root<WebIDL::ArrayBufferView>> pixels)
{
    m_context->make_current();
...
void WebGL2RenderingContextOverloads::read_pixels(WebIDL::Long x, WebIDL::Long y, WebIDL::Long width, WebIDL::Long height, WebIDL::UnsignedLong format, WebIDL::UnsignedLong type, Optional<GC::Root<WebIDL::ArrayBufferView>> pixels)
{
    m_context->make_current();
    ...
    auto span = MUST(get_offset_span<u8>(*pixels.value(), /* src_offset= */ 0));
