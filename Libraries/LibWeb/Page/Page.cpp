...
{
    (void)local_root_navigable()->navigate({ .url = url, .source_document = *local_root_navigable()->active_document(), .history_handling = history_handling, .user_involvement = HTML::UserNavigationInvolvement::BrowserUI });
}
...
{
    (void)local_root_navigable()->navigate({
        .url = url,
        .source_document = *local_root_navigable()->active_document(),
...
void Page::load_html(StringView html, URL::URL const& url)
{
    ...
    (void)local_root_navigable()->navigate(move(params));
}
...
{
    return local_root_navigable()->event_handler().handle_mousewheel(device_to_css_point(position), device_to_css_point(screen_position), button, buttons, modifiers, wheel_delta_x, wheel_delta_y, async_scroll_performed_default_action, async_scroll_operation);
}
...
void Page::invalidate_compositor_wheel_event_listener_state()
{
    ...
    if (!m_async_scrolling_enabled || !local_root_navigable_is_initialized() || !local_root_navigable()->has_compositor_context())
        return;
...
void Page::update_needs_beforeunload_check()
{
    auto needs_beforeunload_check = [&] {
        if (!local_root_navigable_is_initialized())
            return true;

        auto top_level_traversable = this->local_root_navigable();
...
{
    if (!local_root_navigable_is_initialized() || !local_root_navigable()->active_document())
        return;
...
{
    VERIFY(local_root_navigable_is_initialized());
    ...
    for (auto const& document : documents_in_active_window()) {
        ...
        if (document == local_root_navigable()->active_document()) {
...
{
    if (!(m_last_find_in_page_query.has_value() && local_root_navigable_is_initialized()))
        return {};
