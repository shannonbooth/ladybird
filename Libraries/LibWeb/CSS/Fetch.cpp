...
static GC::Ptr<Fetch::Infrastructure::Request> fetch_a_style_resource_impl(StyleResourceURL const& url_value, RuleOrDeclaration css_rule_or_declaration, Fetch::Infrastructure::Request::Destination destination, CorsMode cors_mode)
{
    ...
    request->set_url(parsed_url.value());
    ...
    if (request->mode() == Fetch::Infrastructure::Request::Mode::CORS)
        request->set_referrer(style_resource_base_url(css_rule_or_declaration));

    // FIXME: Spec broken here.
    // css_rule_or_declaration.visit(
...
GC::Ptr<HTML::SharedResourceRequest> fetch_an_external_image_for_a_stylesheet(StyleResourceURL const& url_value, RuleOrDeclaration declartion)
{
    ...
    auto request = fetch_a_style_resource_impl(url_value, declartion, Fetch::Infrastructure::Request::Destination::Image, CorsMode::NoCors);
    if (!request)
        return {};

    // auto document = declartion.visit(
