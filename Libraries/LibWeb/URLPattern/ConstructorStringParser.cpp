...
WebIDL::ExceptionOr<URLPatternInit> ConstructorStringParser::parse(Utf8View const& input)
{
    ...
    while (parser.m_token_index < parser.m_token_list.size()) {
        ...
        dbgln("{}\t| Token@{} (group depth {}) -> {}", parser.state_to_string(), parser.m_token_index, parser.m_group_depth, parser.m_token_list[parser.m_token_index].to_string());
#endif
...
#ifdef URL_PATTERN_DEBUG
    dbgln("Constructor parser parsed:");
    dbgln("{}", parser.m_result.to_string());
#endif
...
String ConstructorStringParser::make_a_component_string() const
{
    ...
    return MUST(String::from_utf8(m_input.unicode_substring_view(component_start_input_index, end_index - component_start_input_index).as_string()));
}
...
static bool protocol_component_matches_a_special_scheme(Component const& protocol_component)
{
    ...
    auto special_scheme_list = URL::special_schemes();

    // 2. For each scheme of special scheme list:
    for (StringView scheme : special_scheme_list) {
...
WebIDL::ExceptionOr<void> ConstructorStringParser::compute_protocol_matches_a_special_scheme_flag()
{
    ...
    auto protocol_component = TRY(Component::compile(protocol_string.code_points(), canonicalize_a_protocol, PatternParser::default_options));
