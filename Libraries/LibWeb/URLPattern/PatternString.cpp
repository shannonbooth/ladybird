...
String generate_a_pattern_string(Vector<Part> const& part_list, PatternParser::Options const& options)
{
    ...
    for (size_t index = 0; index < part_list.size(); ++index) {
        ...
        if (!needs_grouping
            ...
            // 2. Otherwise:
                ...
                needs_grouping = is_ascii_digit(*part.name.code_points().begin());
            }
        }
