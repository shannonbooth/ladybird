...
String URLPatternInit::to_string() const
{
    StringBuilder builder;

    auto do_field = [&builder](StringView field_name, Optional<String> const& maybe_value) {
        if (maybe_value.has_value())
            builder.appendff("{} -> \"{}\"\n", field_name, maybe_value.value());
        else
            builder.appendff("{} -> None\n", field_name);
    };

    do_field("protocol"sv, protocol);
    do_field("username"sv, username);
    do_field("password"sv, password);
    do_field("hostname"sv, hostname);
    do_field("port"sv, port);
    do_field("pathname"sv, pathname);
    do_field("search"sv, search);
    do_field("hash"sv, hash);
    do_field("base_url"sv, base_url);
...
static WebIDL::ExceptionOr<String> process_protocol_for_init(String const& value, PatternProcessType type)
{
...
static WebIDL::ExceptionOr<String> process_hostname_for_init(String const& value, PatternProcessType type)
{
    // 1. If type is "pattern" then return value.
    if (type == PatternProcessType::Pattern)
        return value;
...
static WebIDL::ExceptionOr<String> process_port_for_init(String const& port_value, String const& protocol_value, PatternProcessType type)
{
...
static WebIDL::ExceptionOr<String> process_pathname_for_init(String const& pathname_value, String const& protocol_value, PatternProcessType type)
{
    ...
    if (protocol_value.is_empty() || URL::is_special_scheme(protocol_value))
...
static WebIDL::ExceptionOr<String> process_search_for_init(String const& value, PatternProcessType type)
{
...
static WebIDL::ExceptionOr<String> process_hash_for_init(String const& value, PatternProcessType type)
{
...
WebIDL::ExceptionOr<URLPatternInit> process_a_url_pattern_init(URLPatternInit const& init, PatternProcessType type,
    ...
    Optional<String> const& search, Optional<String> const& hash)
{
    ...
    if (init.base_url.has_value()) {
        ...
        base_url = URL::Parser::basic_parse(init.base_url.value());

        // 2. If baseURL is failure, then throw a TypeError.
        if (!base_url.has_value())
            return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Invalid base URL provided for URLPattern"sv };
        ...
        if (!init.protocol.has_value() && !init.hostname.has_value() && !init.port.has_value() && !init.pathname.has_value() && !init.search.has_value() && !init.hash.has_value()) {
    ...
    // FIXME: Probably spec bug, does not seem to handle a null protocol.
    ...
    if (init.pathname.has_value()) {
        ...
        //    then:
        if (base_url.has_value()
            && !base_url->cannot_be_a_base_url() // FIXME: Update URL code to have concept of opaque path
        ...
        // FIXME: Probable spec bug, need to dig into. Does not seem to handle null protocol.
