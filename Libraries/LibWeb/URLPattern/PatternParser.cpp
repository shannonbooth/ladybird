...
String Part::convert_type_to_string(Part::Type type)
{
    switch (type) {
    case Type::FixedText:
        return "FixedText"_string;
    case Type::Regexp:
        return "Regexp"_string;
    case Type::SegmentWildcard:
        return "SegmentWildcard"_string;
    case Type::FullWildcard:
        return "FullWildcard"_string;
    }
...
String Part::convert_modifier_to_string(Part::Modifier modifier)
{
...
PatternParser::PatternParser(EncodingCallback encoding_callback, String segment_wildcard_regexp)
    ...
    , m_segment_wildcard_regexp(segment_wildcard_regexp)
{
}
...
PatternParser::Options const PatternParser::default_options = {};
...
PatternParser::Options const PatternParser::hostname_options = {
...
PatternParser::Options const PatternParser::pathname_options = {
    .delimiter_code_point = '/',
    .prefix_code_point = '/',
    .ignore_case = false,
};
...
WebIDL::ExceptionOr<void> PatternParser::consume_a_required_token(Token::Type type)
{
    ...
    if (!result)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, MUST(String::formatted("Missing required token '{}' in URL pattern", Token::type_to_string(type))) };
...
WebIDL::ExceptionOr<void> PatternParser::maybe_add_a_part_from_the_pending_fixed_value()
{
...
WebIDL::ExceptionOr<void> PatternParser::add_a_part(String const& prefix, Token const* name_token,
    Token const* regexp_or_wildcard_token, String const& suffix, Token const* modifier_token)
{
    ...
    // Spec-Note: This was a "{foo}" grouping. We add this to the pending fixed value so that it will be combined with
    ...
    // Spec-Note: This was a "{foo}?" grouping. The modifier means we cannot combine it with other text. Therefore we
    ...
    // Spec-Note: Next, we convert the regexp or wildcard token into a regular expression.
    String regexp_value;
    ...
    // Spec-Note: Next, we convert regexp value into a part type. We make sure to go to a regular expression first so
    ...
    // Spec-Note: Next, we determine the part name. This can be explicitly provided by a "name" token or be automatically assigned.
    String name;
    ...
    if (is_a_duplicate_name(name))
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Duplicate name provided in URL pattern"_string }; // FIXME: Improve error

    // 17. Let encoded prefix be the result of running parser’s encoding callback given prefix.
    // Spec-Note: Finally, we encode the fixed text values and create the part.
...
WebIDL::ExceptionOr<Vector<Part>> PatternParser::parse(Utf8View const& input, Options const& options, EncodingCallback encoding_callback)
{
    ...
    while (parser.m_index < parser.m_token_list.size()) {
        ...
        // Spec-Note: If there is a matching group, we need to add the part immediately.
        if (name_token || regexp_or_wildcard_token) {
        ...
        // Spec-Note: If there was no matching group, then we need to buffer any fixed text. We want to collect as
    ...
    for (auto const& part : parser.m_part_list) {
        dbgln("Type {}, Value '{}', Modifier {}, Name '{}', Prefix '{}', Suffix '{}'",
            Part::convert_type_to_string(part.type), part.value, Part::convert_modifier_to_string(part.modifier),
