...
WebIDL::ExceptionOr<Vector<Token>> Tokenizer::tokenize(Utf8View const& input, Tokenizer::Policy policy)
{
#ifdef URL_PATTERN_DEBUG
    dbgln("Tokenizing '{}'", input.as_string());
#endif
...
WebIDL::ExceptionOr<void> Tokenizer::process_a_tokenizing_error(u32 next_position, u32 value_position)
{
    // 1. If tokenizer’s policy is "strict", then throw a TypeError.
    if (m_policy == Policy::Strict)
        return WebIDL::SimpleException { WebIDL::SimpleExceptionType::TypeError, "Error processing a token"sv }; // FIXME: Improve this error!
