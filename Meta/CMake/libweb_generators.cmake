...
function (generate_js_bindings target)
    ...
    # Ninja expects the target name in depfiles to be relative to CMAKE_BINARY_DIR, but our outputs are
    ...
    if (WIN32)
        list(JOIN LIBWEB_ALL_IDL_FILES "\n" idl_file_list)
        file(GENERATE OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/all_bindings_idl_files.txt" CONTENT "${idl_file_list}" NEWLINE_STYLE UNIX)
        set(LIBWEB_ALL_IDL_FILES_ARGUMENT "@${CMAKE_CURRENT_BINARY_DIR}/all_bindings_idl_files.txt")
    endif()
    ...
    add_custom_command(
        ...
        COMMAND "$<TARGET_FILE:Lagom::BindingsGenerator>" -o "Bindings" --depfile "Bindings/all_bindings.d"
                ${depfile_prefix_arg} -b "${LIBWEB_INPUT_FOLDER}" -b "${CMAKE_CURRENT_BINARY_DIR}" ${LIBWEB_ALL_IDL_FILES_ARGUMENT}
        VERBATIM
