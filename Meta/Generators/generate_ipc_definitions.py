...
""")
    ...
    for message in endpoint.messages:
        ...
        out.write(f"\n    virtual {return_type} {message.name}({params}) override\n    {{\n")
        if return_type == "void":
            out.write(
            ...
            out.write(
                f"        if (auto* stub = {stub_accessor}({route.name}))\n            return stub->{message.name}({args});\n"
            )
            outputs = ", ".join([return_type] + [output.type for output in message.outputs])
            out.write(f"        return IPC::empty_response<{outputs}>();\n")
        out.write("    }\n")
