...
def write_html_constructor_steps(
    ...
    autonomous_custom_element_body = (
        '        // Do nothing, as this is the HTMLElement constructor.\n'
        if interface.name == "HTMLElement"
        else '        return vm.throw_completion<JS::TypeError>("Autonomous custom elements can only inherit from HTMLElement"sv);\n'
    )
...
{autonomous_custom_element_body}    }}
    ...
    // 8. Otherwise (i.e., if definition is for a customized built-in element):
        ...
        static auto valid_local_names = MUST(DOM::valid_local_names_for_given_html_element_interface("{interface.name}"sv));
    ...
    if (definition->construction_stack().is_empty()) {{
        ...
        auto element = realm.create<{fully_qualified_name(interface)}>(window.associated_document(), DOM::QualifiedName {{ definition->local_name(), {{}}, Namespace::HTML }});

        // https://webidl.spec.whatwg.org/#internally-create-a-new-object-implementing-the-interface
        TRY(WebIDL::set_prototype_from_new_target<{interface.prototype_class}>(vm, new_target, "{interface.name}"_fly_string, *element));
    ...
    if (!prototype.is_object()) {{
        ...
        prototype = &Bindings::ensure_web_prototype<{interface.prototype_class}>(*function_realm, "{interface.name}"_fly_string);
    }}
