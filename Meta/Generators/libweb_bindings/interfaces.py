...
def write_implementation(out: TextIO, includes: GeneratedIncludes, context: GenerationContext) -> None:
    ...
    if interface.parent_name:
        includes.add_binding(f"{interface.parent_name}")
...
{{
    ...
    object.set_prototype(&ensure_web_constructor<{interface.parent_name}Prototype>(realm, "{interface.parent_name}"_fly_string));
...
{{
    ...
    object.set_prototype(&ensure_web_prototype<{interface.parent_name}Prototype>(realm, "{interface.parent_name}"_fly_string));
    object.define_direct_property(vm.well_known_symbol_to_string_tag(), JS::PrimitiveString::create(vm, "{interface.name}"_string), JS::Attribute::Configurable);
}}
...
def ensure_interface_is_supported(interface: Interface) -> None:
    ...
    if not interface.parent_name:
        raise RuntimeError(f"Unsupported interface '{interface.name}' without a parent interface")
    if interface.member_declarations:
