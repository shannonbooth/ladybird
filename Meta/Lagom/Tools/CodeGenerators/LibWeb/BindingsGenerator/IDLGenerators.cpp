...
static StringView sequence_storage_type_to_cpp_storage_type_name(SequenceStorageType sequence_storage_type)
{
    ...
    case SequenceStorageType::Vector:
        return "Vector"sv;
    case SequenceStorageType::ConservativeVector:
...
static CppType lower_idl_type_to_cpp(Context const& context, Type const& type, TypePosition, Presence, Optional<HashMap<ByteString, ByteString> const&> extended_attributes = { });
...
static ByteString union_type_to_variant(UnionType const& union_type, Context const& context)
{
    ...
    for (size_t type_index = 0; type_index < flattened_types.size(); ++type_index) {
        ...
        auto cpp_type = lower_idl_type_to_cpp(context, type, TypePosition::UnionMember, Presence::Required);
...
static bool type_contains_gc_like_value(Context const&, Type const&, HashTable<ByteString>&);

static bool dictionary_contains_gc_like_value(Context const& context, ByteString const& dictionary_name, HashTable<ByteString>& seen_dictionaries)
{
    auto dictionary = context.dictionaries.find(dictionary_name);
    VERIFY(dictionary != context.dictionaries.end());

    if (seen_dictionaries.contains(dictionary_name))
        return false;

    seen_dictionaries.set(dictionary_name);
    ScopeGuard remove_from_seen = [&] {
        seen_dictionaries.remove(dictionary_name);
    };

    if (!dictionary->value.parent_name.is_empty() && dictionary_contains_gc_like_value(context, dictionary->value.parent_name, seen_dictionaries))
        return true;

    for (auto const& member : dictionary->value.members) {
        if (type_contains_gc_like_value(context, *member.type, seen_dictionaries))
            return true;
    }
...
static bool type_contains_gc_like_value(Context const& context, Type const& type, HashTable<ByteString>& seen_dictionaries)
{
    if (is_platform_object(context, type)
    ...
    if (type.name().is_one_of("sequence"sv, "FrozenArray"sv)) {
        ...
        return type_contains_gc_like_value(context, parameterized_type.parameters().first(), seen_dictionaries);
    }

    if (type.name() == "record"sv) {
        auto const& parameterized_type = as<ParameterizedType>(type);
        return type_contains_gc_like_value(context, parameterized_type.parameters()[0], seen_dictionaries)
            || type_contains_gc_like_value(context, parameterized_type.parameters()[1], seen_dictionaries);
    }

    if (is<UnionType>(type)) {
        for (auto const& member_type : as<UnionType>(type).flattened_member_types()) {
            if (type_contains_gc_like_value(context, member_type, seen_dictionaries))
                return true;
        }
        return false;
    }
    ...
    if (context.dictionaries.contains(type.name()))
        return dictionary_contains_gc_like_value(context, type.name(), seen_dictionaries);
...
{
    HashTable<ByteString> seen_dictionaries;
    return type_contains_gc_like_value(context, type, seen_dictionaries);
}

static SequenceStorageType union_type_sequence_storage_type(UnionType const& union_type, Context const& context)
{
    for (auto const& type : union_type.flattened_member_types()) {
        if (type_contains_gc_like_value(context, type))
            return SequenceStorageType::ConservativeVector;
    }

    return SequenceStorageType::Vector;
}

static CppType gc_ref_type(ByteString referent_type)
{
    return {
        .name = ByteString::formatted("GC::Ref<{}>", referent_type),
        .is_gc_type = true,
        .sequence_storage_type = SequenceStorageType::RootVector,
        .gc_referent_type = move(referent_type),
    };
}
...
{
    return {
        .name = ByteString::formatted("GC::Ptr<{}>", referent_type),
        .is_gc_type = true,
        .is_nullable = true,
        .sequence_storage_type = SequenceStorageType::RootVector,
        .value_access = CppType::ValueAccess::Pointer,
        .gc_referent_type = move(referent_type),
        .null_expression = "{}",
        .default_constructed_expression = "{}",
    };
}

static CppType lower_required_non_nullable_idl_type_to_cpp(Context const& context, Type const& type, [[maybe_unused]] TypePosition position, Optional<HashMap<ByteString, ByteString> const&> extended_attributes)
{
    ...
    if (context.callback_functions.contains(type.name()))
        return gc_ref_type(ByteString { "WebIDL::CallbackType"sv });

    if (type.is_string()) {
        auto is_fly_string = extended_attributes.has_value() && extended_attributes->contains("FlyString"sv);
        if (type.name().contains("Utf16"sv))
            return { .name = is_fly_string ? "Utf16FlyString" : "Utf16String" };
        return { .name = is_fly_string ? "FlyString" : "String" };
    }
    ...
    if (type.name() == "byte")
        return { .name = "WebIDL::Byte" };

    if (type.name() == "octet")
        return { .name = "WebIDL::Octet" };

    if (type.name() == "short")
        return { .name = "WebIDL::Short" };

    if (type.name() == "unsigned short")
        return { .name = "WebIDL::UnsignedShort" };

    if (type.name() == "long")
        return { .name = "WebIDL::Long" };

    if (type.name() == "unsigned long")
        return { .name = "WebIDL::UnsignedLong" };

    if (type.name() == "long long")
        return { .name = "WebIDL::LongLong" };

    if (type.name() == "unsigned long long")
        return { .name = "WebIDL::UnsignedLongLong" };
    ...
    if (type.name() == "any") {
        return {
            ...
            .is_gc_type = true,
            .sequence_storage_type = SequenceStorageType::RootVector,
            .value_access = CppType::ValueAccess::JSValue,
        };
    }
    ...
    if (type.name() == "object")
        return gc_ref_type(ByteString { "JS::Object"sv });

    if (type.name() == "BufferSource")
        return gc_ref_type(ByteString { "WebIDL::BufferSource"sv });

    if (type.name() == "ArrayBufferView")
        return gc_ref_type(ByteString { "WebIDL::ArrayBufferView"sv });

    if (type.name() == "Promise")
        return gc_ref_type(ByteString { "WebIDL::Promise"sv });

    if (type.name().is_one_of("sequence"sv, "FrozenArray"sv)) {
        auto& parameterized_type = as<ParameterizedType>(type);
        auto& sequence_type = parameterized_type.parameters().first();
        auto sequence_cpp_type = lower_idl_type_to_cpp(context, sequence_type, TypePosition::SequenceElement, Presence::Required);
        auto storage_type_name = sequence_storage_type_to_cpp_storage_type_name(sequence_cpp_type.sequence_storage_type);

        return {
            .name = ByteString::formatted("{}<{}>", storage_type_name, sequence_cpp_type.name),
            .is_gc_type = sequence_cpp_type.sequence_storage_type != SequenceStorageType::Vector,
            .needs_heap_for_default_construction = sequence_cpp_type.sequence_storage_type != SequenceStorageType::Vector,
        };
    }
    ...
    if (type.name() == "record") {
        ...
        auto record_key_cpp_type = lower_idl_type_to_cpp(context, record_key_type, TypePosition::AggregateMember, Presence::Required);
        auto record_value_cpp_type = lower_idl_type_to_cpp(context, record_value_type, TypePosition::AggregateMember, Presence::Required);

        if (record_key_cpp_type.sequence_storage_type == SequenceStorageType::ConservativeVector || record_value_cpp_type.sequence_storage_type == SequenceStorageType::ConservativeVector) {
            dbgln("record<{}, {}> needs a conservative hash map storage type that does not exist yet", record_key_cpp_type.name, record_value_cpp_type.name);
            VERIFY_NOT_REACHED();
        }

        if (record_key_cpp_type.sequence_storage_type == SequenceStorageType::RootVector || record_value_cpp_type.sequence_storage_type == SequenceStorageType::RootVector)
            return {
                .name = ByteString::formatted("GC::OrderedRootHashMap<{}, {}>", record_key_cpp_type.name, record_value_cpp_type.name),
                .is_gc_type = true,
                .needs_heap_for_default_construction = true,
            };

        return { .name = ByteString::formatted("OrderedHashMap<{}, {}>", record_key_cpp_type.name, record_value_cpp_type.name) };
    }

    if (is<UnionType>(type)) {
        auto& union_type = as<UnionType>(type);
        auto sequence_storage_type = union_type_sequence_storage_type(union_type, context);
        return {
            .name = union_type_to_variant(union_type, context),
            .is_gc_type = sequence_storage_type != SequenceStorageType::Vector,
            .is_nullable = union_type.includes_undefined() || union_type.includes_nullable_type(),
            .sequence_storage_type = sequence_storage_type,
        };
    }

    if (context.dictionaries.contains(type.name())) {
        auto sequence_storage_type = type_contains_gc_like_value(context, type) ? SequenceStorageType::ConservativeVector : SequenceStorageType::Vector;
        return {
            .name = type.name(),
            .is_gc_type = sequence_storage_type != SequenceStorageType::Vector,
            .sequence_storage_type = sequence_storage_type,
...
static CppType lower_nullable_idl_type_to_cpp(Context const& context, Type const& type, TypePosition position, Optional<HashMap<ByteString, ByteString> const&> extended_attributes)
{
    VERIFY(type.is_nullable());
    VERIFY(!is<UnionType>(type));
    ...
    if (type.name() == "any"sv) {
        return {
            ...
            .sequence_storage_type = SequenceStorageType::RootVector,
            .value_access = CppType::ValueAccess::JSValue,
            .null_expression = "JS::js_null()",
            .default_constructed_expression = "JS::js_undefined()",
        };
    }

    CppType lowered;
    if (auto const* callback_interface = callback_interface_for_type(context, type))
        lowered = gc_ptr_type(interface_cpp_type_name(*callback_interface));
    else if (IDL::is_platform_object(context, type))
        lowered = gc_ptr_type(cpp_type_name(type, context));
    else if (is_javascript_builtin_buffer_source_type(type))
        lowered = gc_ptr_type(ByteString::formatted("JS::{}", type.name()));
    else if (context.callback_functions.contains(type.name()))
        lowered = gc_ptr_type(ByteString { "WebIDL::CallbackType"sv });
    else if (type.name() == "object"sv)
        lowered = gc_ptr_type(ByteString { "JS::Object"sv });
    else if (type.name() == "BufferSource"sv)
        lowered = gc_ptr_type(ByteString { "WebIDL::BufferSource"sv });
    else if (type.name() == "ArrayBufferView"sv)
        lowered = gc_ptr_type(ByteString { "WebIDL::ArrayBufferView"sv });
    else if (type.name() == "Promise"sv)
        lowered = gc_ptr_type(ByteString { "WebIDL::Promise"sv });

    if (!lowered.name.is_empty())
        return lowered;

    auto inner_type = clone_type(type, false);
    auto inner_cpp_type = lower_required_non_nullable_idl_type_to_cpp(context, *inner_type, position, extended_attributes);
    return {
        .name = ByteString::formatted("Optional<{}>", inner_cpp_type.name),
        .is_nullable = true,
        .value_access = CppType::ValueAccess::OptionalValue,
        .null_expression = "{}",
        .default_constructed_expression = "{}",
    };
}

static CppType with_optional_presence(CppType lowered)
{
    if (lowered.value_access == CppType::ValueAccess::JSValue)
        return lowered;

    if (lowered.value_access == CppType::ValueAccess::Pointer || lowered.is_nullable) {
        lowered.name = ByteString::formatted("Optional<{}>", lowered.name);
        lowered.is_optional_presence = true;
        lowered.value_access = CppType::ValueAccess::OptionalValue;
        lowered.sequence_storage_type = SequenceStorageType::Vector;
        lowered.default_constructed_expression = "{}";
        return lowered;
    }

    if (!lowered.gc_referent_type.is_empty() && lowered.value_access == CppType::ValueAccess::Direct) {
        lowered.name = ByteString::formatted("GC::Ptr<{}>", lowered.gc_referent_type);
        lowered.is_nullable = true;
        lowered.is_optional_presence = true;
        lowered.value_access = CppType::ValueAccess::Pointer;
        lowered.sequence_storage_type = SequenceStorageType::RootVector;
        lowered.default_constructed_expression = "{}";
        return lowered;
    }
...
static CppType lower_idl_type_to_cpp(Context const& context, Type const& type, TypePosition position, Presence presence, Optional<HashMap<ByteString, ByteString> const&> extended_attributes)
{
    auto lowered = type.is_nullable() && !is<UnionType>(type)
        ? lower_nullable_idl_type_to_cpp(context, type, position, extended_attributes)
        : lower_required_non_nullable_idl_type_to_cpp(context, type, position, extended_attributes);

    if (presence == Presence::Required)
        return lowered;

    if (lowered.is_nullable && presence != Presence::Required)
        return lowered;

    return with_optional_presence(move(lowered));
}
...
{
    return lower_idl_type_to_cpp(context, type, TypePosition::StackValue, Presence::Required, extended_attributes);
}
...
static ByteString string_default_value_expression(ByteString const& cpp_type, ByteString const& default_value)
{
...
static Presence dictionary_member_presence(DictionaryMember const& member)
{
    return !member.required && !member.default_value.has_value()
        ? Presence::OptionalDictionaryMember
        : Presence::Required;
}

static CppType lower_dictionary_member_cpp_type(Context const& context, DictionaryMember const& member)
{
    return lower_idl_type_to_cpp(context, *member.type, TypePosition::AggregateMember, dictionary_member_presence(member), member.extended_attributes);
}

static ByteString default_construct_cpp_type_expression(CppType const& cpp_type)
{
    if (cpp_type.needs_heap_for_default_construction)
        return ByteString::formatted("{} {{ vm.heap() }}", cpp_type.name);

    return ByteString::formatted("{} {{}}", cpp_type.name);
}

static bool dictionary_needs_heap_for_default_construction(Context const& context, ByteString const& dictionary_name)
{
    auto dictionary = context.dictionaries.find(dictionary_name);
    VERIFY(dictionary != context.dictionaries.end());

    if (!dictionary->value.parent_name.is_empty() && dictionary_needs_heap_for_default_construction(context, dictionary->value.parent_name))
        return true;
...
static Optional<ByteString> dictionary_heap_default_initializers(Context const& context, ByteString const& dictionary_name)
{
    auto dictionary = context.dictionaries.find(dictionary_name);
    VERIFY(dictionary != context.dictionaries.end());
    ...
    if (!dictionary->value.parent_name.is_empty()) {
        if (auto parent_initializers = dictionary_heap_default_initializers(context, dictionary->value.parent_name); parent_initializers.has_value())
    ...
    for (auto const& member : dictionary->value.members) {
        if (!dictionary_member_needs_builder(context, member))
            continue;
        initializers.append(ByteString::formatted(".{} = {}", make_input_acceptable_cpp(member.name.to_snakecase()), default_construct_cpp_type_expression(lower_dictionary_member_cpp_type(context, member))));
    }
...
static ByteString type_default_expression(Context const& context, Type const& type, Optional<ByteString> const& default_value = { }, Optional<HashMap<ByteString, ByteString> const&> extended_attributes = { });
...
static ByteString type_default_expression(Context const& context, Type const& type, Optional<ByteString> const& default_value, Optional<HashMap<ByteString, ByteString> const&> extended_attributes)
{
    auto cpp_type = lower_idl_type_to_cpp(context, type, TypePosition::StackValue, Presence::Required, extended_attributes);
    ...
    if (is<UnionType>(type)) {
        ...
        if (!default_value.has_value()) {
            ...
            return type_default_expression(context, *types.first());
        }
        ...
        return type_default_expression(context, union_member_type_for_default_value(context, union_type, *default_value), default_value);
    }
    ...
    if (!default_value.has_value()) {
        ...
        if (type.name().is_one_of("sequence"sv, "FrozenArray"sv, "record"sv))
            return default_construct_cpp_type_expression(cpp_type);

        if (context.dictionaries.contains(type.name())) {
            if (auto initializers = dictionary_heap_default_initializers(context, type.name()); initializers.has_value())
    ...
    auto inner_type = clone_type(type, false);
    auto inner_cpp_type = lower_idl_type_to_cpp(context, *inner_type, TypePosition::StackValue, Presence::Required, extended_attributes);
    ...
    if (type.is_string())
        return string_default_value_expression(inner_cpp_type.name, explicit_default_value);
    ...
    if (type.name().is_one_of("sequence"sv, "FrozenArray"sv)) {
        ...
        return default_construct_cpp_type_expression(inner_cpp_type);
    }
...
static void generate_optional_to_cpp(SourceGenerator& generator, ParameterType& parameter, ByteString const& js_name, ByteString const& js_suffix, ByteString const& cpp_name, Context const& context, Optional<ByteString> optional_default_value, size_t recursion_depth)
{
    ...
    auto optional_presence = optional_default_value.has_value() ? Presence::Required : Presence::OptionalArgument;
    auto optional_cpp_type = lower_idl_type_to_cpp(context, *parameter.type, TypePosition::StackValue, optional_presence, parameter.extended_attributes);
    ...
    if (optional_default_value == "{}"sv && context.dictionaries.contains(parameter.type->name()) && dictionary_needs_heap_for_default_construction(context, parameter.type->name())) {
    ...
    if (optional_default_value.has_value())
        initial_value = type_default_expression(context, *parameter.type, optional_default_value, parameter.extended_attributes);
    else if (optional_cpp_type.value_access == CppType::ValueAccess::JSValue)
        initial_value = type_default_expression(context, *parameter.type, { }, parameter.extended_attributes);
...
static void generate_nullable_to_cpp(SourceGenerator& generator, ParameterType const& parameter, ByteString const& js_name, ByteString const& js_suffix, ByteString const& cpp_name, Context const& context, size_t recursion_depth)
{
    ...
    auto nullable_cpp_type = lower_idl_type_to_cpp(context, *parameter.type, TypePosition::StackValue, Presence::Required, parameter.extended_attributes).name;
...
static void generate_variadic_to_cpp(SourceGenerator& generator, ParameterType& parameter, ByteString const& cpp_name, Context const& context, size_t recursion_depth)
{
    ...
    if (variadic_cpp_type.sequence_storage_type != SequenceStorageType::Vector) {
...
static void generate_dictionary_to_cpp(SourceGenerator& generator, Context const& context, IDL::Dictionary const& dictionary, ByteString dictionary_name)
{
    ...
    Vector<DictionaryMember const*> builder_members;
    for (auto const& member : dictionary.members) {
        if (dictionary_member_needs_builder(context, member))
            builder_members.append(&member);
    }
    ...
    if (builder_members.is_empty()) {
        if (auto initializers = dictionary_heap_default_initializers(context, current_dictionary_name); initializers.has_value()) {
            generator.set("dictionary.default_initializers", *initializers);
            generator.append(R"~~~(
    @parameter.type.name.normalized@ @cpp_name@ { @dictionary.default_initializers@ };
)~~~");
        } else {
            generator.append(R"~~~(
    @parameter.type.name.normalized@ @cpp_name@ {};
        ...
        Vector<ByteString> builder_initializers;

        if (!dictionary.parent_name.is_empty()) {
            if (auto parent_initializers = dictionary_heap_default_initializers(context, dictionary.parent_name); parent_initializers.has_value())
                builder_initializers.append(ByteString::formatted("{} {{ {} }}", dictionary.parent_name, *parent_initializers));
        }

        for (auto const* member : builder_members) {
            generator.set("member_key", member->name);
            auto member_js_name = make_input_acceptable_cpp(member->name.to_snakecase());
            auto member_builder_name = ByteString::formatted("{}_builder_{}", member_js_name, i);
            ...
            generator.set("member.builder_type", ByteString::formatted("Optional<{}>", dictionary_member_cpp_type(context, *member)));
            generator.set("member_builder_name", member_builder_name);
            generator.set("member_value_name", member_value_name);
            generator.set("member_property_value_name", member_property_value_name);
            generator.append(R"~~~(
    @member.builder_type@ @member_builder_name@;
    ...
    if (@member_property_value_name@.is_undefined())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::MissingRequiredProperty, "@member_key@");
            ...
            auto builder_member = *member;
            generate_to_cpp(generator, builder_member, member_property_value_name, "", member_value_name, context, !member->required, member->default_value);

            generator.append(R"~~~(
    @member_builder_name@ = @member_value_name@;
)~~~");
            builder_initializers.append(ByteString::formatted(".{} = {}.release_value()", member_js_name, member_builder_name));
            i++;
        }

        generator.set("dictionary.builder_initializers", MUST(String::join(", "sv, builder_initializers)));
        generator.append(R"~~~(
    @parameter.type.name.normalized@ @cpp_name@ { @dictionary.builder_initializers@ };
)~~~");
    }
    ...
    while (true) {
        ...
        for (auto& member : members) {
            if (current_dictionary == &dictionary && dictionary_member_needs_builder(context, member))
                continue;
...
static void generate_record_to_cpp(SourceGenerator& scoped_generator, IDL::ParameterizedType const& type, ByteString const& cpp_name, Context const& context, size_t recursion_depth)
{
    ...
    record_generator.set("record.heap_initializer", record_cpp_type.needs_heap_for_default_construction ? " { vm.heap() }"sv : ""sv);
    ...
    record_generator.append(R"~~~(
    @record.type@ @cpp_name@@record.heap_initializer@;
...
static void generate_union_to_cpp(SourceGenerator& scoped_generator, ParameterType& parameter, ByteString const& js_name, ByteString const& js_suffix, ByteString const& cpp_name, Context const& context, size_t recursion_depth)
{
    ...
    //    2. If types includes object, then return the IDL value that is a reference to the object V.
    bool includes_array_buffer = any_of(types, [](auto const& type) { return type->name() == "ArrayBuffer"; });
    if (includes_array_buffer) {
        union_generator.append(R"~~~(
            if (is<JS::ArrayBuffer>(@js_name@@js_suffix@_object))
                return static_cast<JS::ArrayBuffer&>(@js_name@@js_suffix@_object);
)~~~");
    ...
    //    2. If types includes object, then return the IDL value that is a reference to the object V.
    bool includes_data_view = any_of(types, [](auto const& type) { return type->name() == "DataView"; });
    if (includes_data_view) {
        union_generator.append(R"~~~(
            if (is<JS::DataView>(@js_name@@js_suffix@_object))
                return static_cast<JS::DataView&>(@js_name@@js_suffix@_object);
)~~~");
    } else if (includes_object) {
        union_generator.append(R"~~~(
            if (is<JS::DataView>(@js_name@@js_suffix@_object))
        ...
        dictionary_generator.append(R"~~~(
    {
        ...
        while (true) {
            for (auto& member : current_dictionary->members) {
                ...
                auto member_presence = !member.required && !member.default_value.has_value()
                    ? Presence::OptionalDictionaryMember
                    : Presence::Required;
                auto member_cpp_type = lower_idl_type_to_cpp(context, *member.type, TypePosition::AggregateMember, member_presence, member.extended_attributes);
...
static ByteString dictionary_member_cpp_type(Context const& context, DictionaryMember const& member)
{
    return lower_dictionary_member_cpp_type(context, member).name;
}

static bool type_default_value_needs_heap_for_construction(Context const& context, Type const& type, ByteString const& default_value)
{
    if (is<UnionType>(type)) {
        if (default_value == "null"sv)
            return false;
        return type_default_value_needs_heap_for_construction(context, union_member_type_for_default_value(context, as<UnionType>(type), default_value), default_value);
    }

    if ((type.name().is_one_of("sequence"sv, "FrozenArray"sv) && default_value == "[]"sv)
        || (type.name() == "record"sv && default_value == "{}"sv)) {
        return lower_idl_type_to_cpp(context, type, TypePosition::StackValue, Presence::Required).needs_heap_for_default_construction;
    }

    if (context.dictionaries.contains(type.name()) && default_value == "{}"sv)
        return dictionary_needs_heap_for_default_construction(context, type.name());
...
static bool dictionary_member_needs_builder(Context const& context, DictionaryMember const& member)
{
    ...
    auto member_cpp_type = lower_dictionary_member_cpp_type(context, member);
    if (member_cpp_type.is_optional_presence)
        return false;

    if ((!member_cpp_type.gc_referent_type.is_empty() && member_cpp_type.value_access == CppType::ValueAccess::Direct)
        || member_cpp_type.needs_heap_for_default_construction) {
        return true;
    }
    ...
    if (is<UnionType>(*member.type)
        ...
        && idl_type_name_to_cpp_type(*member.type, context).sequence_storage_type != SequenceStorageType::Vector) {
        return true;
    }

    if (member.default_value.has_value() && type_default_value_needs_heap_for_construction(context, *member.type, *member.default_value))
        return true;
...
static ByteString dictionary_member_initializer(Context const& context, DictionaryMember const& member)
{
    ...
    if (member.default_value.has_value()) {
        ...
        auto default_value_expression = type_default_expression(context, *member.type, member.default_value, member.extended_attributes);
    ...
    if (is<UnionType>(*member.type) && !member_cpp_type.is_optional_presence)
        return ByteString::formatted(" {{ {} }}", type_default_expression(context, *member.type, { }, member.extended_attributes));
