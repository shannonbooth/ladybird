...
static ErrorOr<void> run_test_batch(
    ...
    size_t total_test_count,       // For logging progress relative to all tests (parallel + serial)
    size_t& global_test_index,     // Tracks the cumulative test index across batches
    size_t& total_tests_remaining, // Tracks the total number of tests remaining across batches
    ...
    Application const& app)
{
    ...
    for (auto [view_id, view] : enumerate(views)) {
        ...
        view->test_promise().when_resolved([&, run_next_test, view_id](auto result) {
            ...
            case TestResult::Pass:
                ++test_stats.pass_count;
                break;
            case TestResult::Fail:
                ++test_stats.fail_count;
                break;
            case TestResult::Timeout:
                ++test_stats.timeout_count;
                break;
            case TestResult::Crashed:
                ++test_stats.crashed_count;
                break;
            case TestResult::Skipped:
                ++test_stats.skipped_count;
                break;
            }

            if (result.result != TestResult::Pass)
                test_stats.non_passing_tests.append(move(result));
...
static ErrorOr<int> run_tests(Core::AnonymousBuffer const& theme, Web::DevicePixelSize window_size)
{
    ...
    auto result_or_rejection = run_test_batch(views, parallel_tests, total_tests, global_test_index, total_tests_remaining, test_stats, app);
    if (!result_or_rejection.is_error()) {
        // Only use a single view so that the serial tests run one web view at a time.
        auto single_view = views.span().slice(1);
        result_or_rejection = run_test_batch(single_view, serial_tests, total_tests, global_test_index, total_tests_remaining, test_stats, app);
    }
    ...
    outln("==========================================================");
    outln("Pass: {}, Fail: {}, Skipped: {}, Timeout: {}, Crashed: {}", test_stats.pass_count, test_stats.fail_count, test_stats.skipped_count, test_stats.timeout_count, test_stats.crashed_count);
    outln("==========================================================");

    for (auto const& non_passing_test : test_stats.non_passing_tests) {
    ...
    return test_stats.fail_count + test_stats.timeout_count + test_stats.crashed_count + total_tests_remaining;
}
